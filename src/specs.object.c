...
int blind_boots(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (IS_FIGHTING(ch) && !number(0, 24))
  {
    ...
    if (affected_by_spell(vict, SPELL_BLINDNESS) || (GET_RACE(vict) == RACE_DRAGON) || (GET_RACE(vict) == RACE_DEMON) || (GET_RACE(vict) == RACE_DEVIL) || IS_IMMATERIAL(vict) || IS_ELEMENTAL(vict) || EYELESS(vict) || IS_ELITE(vict))
    {
      return TRUE;
    }
...
int totem_of_mastery(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  e_pos = ((obj->loc.wearing->equipment[HOLD] == obj) ? HOLD : (obj->loc.wearing->equipment[WIELD] == obj)          ? WIELD
