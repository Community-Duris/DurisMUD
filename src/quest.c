...
int find_quester_id(int num) { return (binary_search(num, 0, number_of_quests - 1)); }
		...
		switch (gp->goal_type)
		{
			case QUEST_GOAL_ITEM:
				...
				else
				{
					...
					act("$n places $p on the ground in front of you.", FALSE, mob, obj, pl, TO_VICT);
					act("$n places $p on the ground in front of $N.", FALSE, mob, obj, pl, TO_NOTVICT);
				}
				statuslog(58, "%s was rewarded %s [%d] by %s", GET_NAME(pl), obj->short_description, obj_index[obj->R_num].virtual_number, mob->player.short_descr);
				sql_log(pl, QUESTLOG, "Rewarded %s [%d] by %s", obj->short_description, obj_index[obj->R_num].virtual_number, mob->player.short_descr);
			...
			case QUEST_GOAL_SKILL:
				...
				{
					act("$n says to you, 'Come back when you are more powerful.'", FALSE, mob, 0, pl, TO_VICT);
					break;
				}
...
int quester(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (((!CAN_SEE(ch, pl) || !CAN_SEE(pl, ch)) && (GET_LEVEL(pl) < MINLVLIMMORTAL)) || IS_FIGHTING(ch) || IS_DESTROYING(ch) || (GET_STAT(ch) <= STAT_SLEEPING))
		return (FALSE);
	...
	{ /* player asked about quest */
		...
		if (!*name || !*Gbuf1 || (!(vict = get_char_room_vis(pl, name)) && (GET_LEVEL(pl) < MINLVLIMMORTAL)) || (vict != ch))
			return (FALSE);
				...
				switch (letterStrn[0])
				{
					...
					case 'D':
						if (quest_index[number_of_quests].quest_complete)
						{
							...
							quest_index[number_of_quests].quest_complete->disappear_message = fread_string(quest_f);
						}
						else
						{
...
bool has_quest_ask(int qi)
{
	...
	for (qdata = quest_index[qi].quest_message; qdata; qdata = qdata->next)
	{
		...
		if ((sscanf(qdata->key_words, QC_ACTION " %d", &arg) == 1) || is_abbrev(QC_UNBLOCK " ", qdata->key_words))
		{
			continue;
		}
		else
