...
bool can_relocate_to(P_char ch, P_char victim)
{
  ...
  if (!(victim) || !(location) || !can_enter_room(ch, location, FALSE) || ((racewar(ch, victim) || IS_NPC(victim) || IS_ROOM(ch->in_room, ROOM_SINGLE_FILE) || IS_ROOM(victim->in_room, ROOM_SINGLE_FILE)) && !IS_TRUSTED(ch)))
  {
                           ...
                           P_char victim, P_obj obj)
{
  ...
  send_to_char("You send a &+Rr&+Ba&+Yi&+Gn&+Cb&n&+co&+bw&n of prismatic spray from your hands.\n",
               ch);
  act("$n sends a &+Rr&+Ba&+Yi&+Gn&+Cb&n&+co&+bw&n of prismatic spray from $s hands!",
                       ...
                       P_char victim, P_obj obj)
{
  ...
  {
    if ((victim) && (ch) && failed_save && !affected_by_spell(victim, SPELL_CHILL_TOUCH) && !IS_ELITE(victim) && !IS_GREATER_RACE(victim) && !IS_AFFECTED3(victim, AFF3_COLDSHIELD) && !IS_AFFECTED4(victim, AFF4_ICE_AURA))
    {
                                ...
                                P_obj obj)
{
  ...
  { /* Under control */
    act("$N says 'I shall serve you for a short time $n, and then I shall have you!'",
        TRUE, ch, 0, mob, TO_ROOM);
    act("$N says 'I shall serve you for a short time, before taking your soul!'",
        TRUE, ch, 0, mob, TO_CHAR);
                      ...
                      P_char victim, P_obj obj)
{
  ...
  act("&+gAn &+Genchanting &+Csong fills the area... &+git tugs on your &+Csoul.",
...
void spell_elemental_aura(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
  ...
  if (affected_by_spell(ch, SPELL_ELEMENTAL_AURA) || IS_AFFECTED2(ch, AFF2_EARTH_AURA) || IS_AFFECTED2(ch, AFF2_WATER_AURA) || IS_AFFECTED2(ch, AFF2_FIRE_AURA) || IS_AFFECTED2(ch, AFF2_AIR_AURA) || IS_AFFECTED4(ch, AFF4_ICE_AURA))
  {
                      ...
                      P_char victim, P_obj obj)
{
  ...
  if (IS_TRUSTED(victim) || racewar(ch, victim) || !(world[ch->in_room].zone == world[victim->in_room].zone) || IS_NPC(victim) || (IS_MAP_ROOM(ch->in_room) && IS_MAP_ROOM(victim->in_room) && (calculate_map_distance(ch->in_room, victim->in_room) > (distance * distance))))
  {
    send_to_char("&+CYou failed.\n", ch);
    return;
  }
...
void spell_dimension_door(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
  ...
  if (IS_AFFECTED3(victim, AFF3_NON_DETECTION) || IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) || IS_HOMETOWN(ch->in_room) || world[ch->in_room].sector_type == SECT_OCEAN || world[victim->in_room].sector_type == SECT_CASTLE || world[victim->in_room].sector_type == SECT_CASTLE_WALL || world[victim->in_room].sector_type == SECT_CASTLE_GATE)
  {
    send_to_char("&+cYou failed.\n", ch);
    return;
  }
                         ...
                         P_char victim, P_obj obj)
{
  ...
  if (ch == victim)
  {
    act("&+bThe purifying power of the &+Bwater&+b flows through your body.&n",
        FALSE, ch, 0, victim, TO_CHAR);
  }
  else
  {
                   ...
                   P_obj obj)
{
  ...
  {
    act("&+cA &+Ctranslucent&n&+c field flashes around $n&n&+c, then vanishes.",
        TRUE, victim, 0, 0, TO_ROOM);
                ...
                P_obj obj)
{
  ...
  if (obj)
  {
    if (IS_SET(obj->extra_flags, ITEM_NOIDENTIFY))
    {
      ...
      {
        send_to_char("You recall no legends nor stories ever told about this item.",
                     ch);
        return;
      }
    }
...
void spell_regeneration(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
  ...
  if (affected_by_spell(victim, SPELL_ACCEL_HEALING) || affected_by_spell(victim, SKILL_REGENERATE) || affected_by_spell(victim, SPELL_REGENERATION) || affected_by_spell(victim, SPELL_PACTUM_SERPENTIS))
  {
                      ...
                      P_char victim, P_obj obj)
{
  ...
  act("&+m$n&n&+m snaps $s fingers, and a cloud of purple smoke billows forth!",
      TRUE, ch, 0, 0, TO_ROOM);
  act("&+mYou snap your fingers, causing a cloud of purple smoke to billow forth.",
      TRUE, ch, 0, 0, TO_CHAR);
...
void astral_banishment(P_char ch, P_char victim, int hwordtype, int level)
{
  ...
  if (GET_RACE(victim) == (hwordtype == BANISHMENT_HOLY_WORD ? RACE_GITHYANKI : RACE_GITHZERAI) && number(0, 100) < 2 + BOUNDED(-2, level - lev, 2) && !IS_HOMETOWN(ch->in_room) && !IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) && !(world[ch->in_room].sector_type == SECT_OCEAN))
  {
    if (has_band)
    {
...
void spell_pword_stun(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
  ...
  if (!IS_ALIVE(victim) || GET_HIT(victim) < 1 || !IS_ALIVE(ch) || IS_TRUSTED(victim) || IS_ZOMBIE(victim) || GET_RACE(victim) == RACE_GOLEM || GET_RACE(victim) == RACE_PLANT || GET_RACE(victim) == RACE_CONSTRUCT || IS_GREATER_RACE(victim))
  {
...
void spell_oldjudgement(int level, P_char ch, P_char victim, P_obj obj)
{
  ...
  if (GET_LEVEL(ch) < MINLVLIMMORTAL)
  {
    ...
    {
      send_to_char("You have angered your god in some way, and refuses your request for aid!",
                   ch);
      return;
    }
  }
...
void spell_judgement(int level, P_char ch, char *arg, int type, P_char vict, P_obj obj)
{
  ...
  if (GET_LEVEL(ch) < MINLVLIMMORTAL)
  {
    ...
    if ((GET_ALIGNMENT(ch) > 899) && (GET_ALIGNMENT(ch) < 980))
    {
      send_to_char("You have angered your god in some way, and he refuses your request for aid!",
                   ch);
      return;
    }
  }
...
void spell_accel_healing(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
  ...
  if (affected_by_spell(victim, SKILL_REGENERATE) ||
      affected_by_spell(victim, SPELL_REGENERATION) || affected_by_spell(victim, SPELL_PACTUM_SERPENTIS))
  {
      ...
      ((to_room == NOWHERE) || (to_room == ch->in_room) ||
       ...
       (victim &&
        ...
        (IS_TRUSTED(victim) ||
         ...
         (IS_SET(victim->specials.act2, PLR2_NOLOCATE) && !is_linked_to(ch, victim, LNK_CONSENT)) ||
         (IS_PC(victim) && IS_SET(victim->specials.act2, PLR2_NOLOCATE) && !is_introd(victim, ch)) ||
         (racewar(ch, victim) && !race_portal_check(ch, victim))) /* check used if victim is set */
                         ...
                         P_char victim, P_obj obj)
{
  ...
  if (!found || tobj->loc_p != LOC_ROOM)
  {
    act("&+L$n attempts to call upon the spirit realm, but is unable to make a connection.&N",
        TRUE, ch, 0, 0, TO_ROOM);
    act("&+gThe &+Gspirit &+grealm fails to answer your call for help.&N",
        FALSE, ch, 0, 0, TO_CHAR);
    return;
  }
  ...
  act("&+G$n's &+gbody begins to shake violently as if possessed by some &+Gother worldly &+gpower. With a blast of &+Gghastly &+Yflames&+g a portal suddenly materializes before them.&N",
      TRUE, ch, 0, 0, TO_ROOM);
  act("&+gYour body calls out to the &+Gspirit&+g realm, begging for assistance.  Every fiber within your body begins to &+Gtingle&+g as a &+Gghastly &+gessence begins to fill the room. Suddenly and without warning, a &+Gmystic &+gportal materializes before you, beckoning you to enter.&N",
      FALSE, ch, 0, 0, TO_CHAR);
  obj_from_room(tobj);
...
void do_soulbind(P_char ch, char *argument, int cmd)
{
  ...
  if (IS_TRUSTED(ch) ? *gbuf2 : *gbuf1)
  {
    ...
    if ((obj->type == ITEM_CONTAINER || obj->type == ITEM_STORAGE || obj->type == ITEM_TREASURE || obj->type == ITEM_POTION || obj->type == ITEM_TELEPORT || obj->type == ITEM_WAND || obj->type == ITEM_KEY || obj->contains || obj->type == ITEM_FOOD || IS_OBJ_STAT2(obj, ITEM2_STOREITEM) || IS_OBJ_STAT2(obj, ITEM2_SOULBIND) || IS_OBJ_STAT(obj, ITEM_NOSELL) || IS_SET(obj->extra_flags, ITEM_ARTIFACT)))
    {
                            ...
                            P_char victim, P_obj obj)
{
  ...
  if (affected_by_spell(ch, SPELL_ACCEL_HEALING) || affected_by_spell(ch, SKILL_REGENERATE) || affected_by_spell(ch, SPELL_REGENERATION) || affected_by_spell(ch, SPELL_PACTUM_SERPENTIS) || IS_AFFECTED3(ch, AFF3_GR_SPIRIT_WARD))
  {
