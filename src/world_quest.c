...
void quest_kill(P_char ch, P_char quest_mob)
{
	...
	if (ch->only.pc->quest_active != 1 || IS_PC(quest_mob) || (GET_VNUM(quest_mob) != ch->only.pc->quest_mob_vnum))
	{
		return;
	}
...
void do_quest(P_char ch, char *args, int cmd)
{
	...
	if (*name)
	{
		...
		else if (isname(name, "stat") && IS_TRUSTED(ch))
		{
			...
			snprintf(buf,
			         ...
			         pcdata->quest_map_room,
			         YESNO(pcdata->quest_map_bought));
			send_to_char(buf, ch);
	...
	if (*name)
		if (isname(name, "share"))
		{
			...
			if (ch->only.pc->quest_level < GET_LEVEL(victim) - get_property("world.quest.level.high.value", 6))
			{
				send_to_char("They are much too experienced for that quest!\r\n", ch);
				return;
			}

			if (ch->only.pc->quest_level > GET_LEVEL(victim) + get_property("world.quest.level.low.value", 6))
			{
...
bool createQuest(P_char ch, P_char giver)
{
	...
	for (int z = 0; z < 10; z++)
	{
		...
		for (int m = 0; m < 3; m++)
		{
			...
			if (quest_mob > 0 && ((QUEST_TYPE == FIND_AND_KILL && mob_index[real_mobile(quest_mob)].number <= 1) || (QUEST_TYPE == FIND_AND_ASK && mob_index[real_mobile(quest_mob)].number < 1)))
			{
				quest_mob = -1;
			}
	...
	*/
	...
	for (i = 0; i <= top_of_objt; i++)
	{
		if ((obj_index[i].virtual_number >= world[z_num->real_bottom].number) && (obj_index[i].virtual_number <= world[z_num->real_top].number))
		{
			...
			{
				if (IS_SET(t_obj->extra_flags, ITEM_NORENT) || IS_SET(t_obj->str_mask, STRUNG_KEYS) || !IS_SET(t_obj->wear_flags, ITEM_TAKE) ||
				    IS_SET(t_obj->extra_flags, ITEM_TRANSIENT)
				    // These are _allowed_ types
				    || (t_obj->type != ITEM_WAND && t_obj->type != ITEM_STAFF && t_obj->type != ITEM_ARMOR && t_obj->type != ITEM_WORN && t_obj->type != ITEM_BOOK && t_obj->type != ITEM_QUIVER &&
				        t_obj->type != ITEM_INSTRUMENT && t_obj->type != ITEM_SPELLBOOK && t_obj->type != ITEM_TOTEM && t_obj->type != ITEM_SHIELD && t_obj->type != ITEM_FIREWEAPON &&
				        t_obj->type != ITEM_WEAPON && t_obj->type != ITEM_POTION) ||
				    IS_SET(t_obj->bitvector, AFF_STONE_SKIN) || IS_SET(t_obj->bitvector, AFF_HIDE) || IS_SET(t_obj->bitvector, AFF_SNEAK) || IS_SET(t_obj->bitvector, AFF_FLY) ||
				    IS_SET(t_obj->bitvector, AFF4_NOFEAR) || IS_SET(t_obj->bitvector2, AFF2_AIR_AURA) || IS_SET(t_obj->bitvector2, AFF2_EARTH_AURA) ||
				    IS_SET(t_obj->bitvector3, AFF3_INERTIAL_BARRIER) || IS_SET(t_obj->bitvector3, AFF3_REDUCE) || IS_SET(t_obj->bitvector2, AFF2_GLOBE) || IS_SET(t_obj->bitvector, AFF_HASTE) ||
				    IS_SET(t_obj->bitvector, AFF_DETECT_INVISIBLE) || IS_SET(t_obj->bitvector4, AFF4_DETECT_ILLUSION) || IS_NOSHOW(t_obj) || GET_OBJ_WEIGHT(t_obj) > 99 || IS_ARTIFACT(t_obj) ||
				    isname("_noquest_", t_obj->name) || IS_OBJ_STAT2(t_obj, ITEM2_QUESTITEM))
				{
...
void getQuestZoneList(P_char ch, vector<int> &valid_zones)
{
	...
	{
		if (isInvalidQuestZone(zone_table[zone_count].number) || zone_table[zone_count].hometown != 0 || zone_table[zone_count].avg_mob_level < 0)
			continue;

		if (zone_table[zone_count].avg_mob_level >= (GET_LEVEL(ch) + 5) || zone_table[zone_count].avg_mob_level <= (GET_LEVEL(ch) - 7))
			continue;
...
int suggestQuestMob(int zone_num, P_char ch, int QUEST_TYPE)
{
	...
	{
		if ((mob_index[i].virtual_number >= world[zone_table[zone_num].real_bottom].number) && (mob_index[i].virtual_number <= world[zone_table[zone_num].real_top].number))
		{
			...
			if ((t_mob = read_mobile(mob_index[i].virtual_number, VIRTUAL)))
			{
				...
				if (GET_LEVEL(t_mob) < MAX_LEVEL && GET_LEVEL(t_mob) > GET_LEVEL(ch) - 5 && KIND_OF_QUEST == FIND_AND_KILL || GET_LEVEL(t_mob) < MAX_LEVEL && KIND_OF_QUEST == FIND_AND_ASK)
				{
					if (mob_index[i].number == 2 && KIND_OF_QUEST == FIND_AND_ASK || KIND_OF_QUEST == FIND_AND_KILL && mob_index[i].number > 2)
					{
						...
						if (aggressive_to(t_mob, ch) && KIND_OF_QUEST == FIND_AND_ASK || !CAN_SPEAK(t_mob) && KIND_OF_QUEST == FIND_AND_ASK)
						{
...
int calc_zone_mob_level()
{
	...
	for (P_char tch = character_list; tch; tch = tch->next)
	{
		...
		avg_mob_level[GET_ZONE(tch)] = ((avg_mob_level[GET_ZONE(tch)] * mob_count[GET_ZONE(tch)]) + (float)GET_LEVEL(tch)) / (mob_count[GET_ZONE(tch)] + 1.0);
