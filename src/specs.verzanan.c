...
{
	static char piergeiron_open_path[] = "W0000l4mnop52333eO33221q4Egh53001O1111222S.";

	static char piergeiron_close_path[] = "W0001101ai000000000bj1111111AC333333300c00000BC22222k22333333FC111111d222222222d3233222S.";
	...
	if (!move)
	{
		if (ch->in_room != real_room(3306))
		{
			if ((GET_STAT(ch) > STAT_INCAP) && !IS_FIGHTING(ch))
				if (GET_STAT(ch) == STAT_SLEEPING)
					act("$n wakes up, looking extremely annoyed.", TRUE, ch, 0, 0, TO_ROOM);
			SET_POS(ch, POS_STANDING + STAT_NORMAL);
			act("$n says, 'Damn, lost again, and almost time for work too!", TRUE, ch, 0, 0, TO_ROOM);
			act("$n pulls a small device from a pocket and presses a button on it.", TRUE, ch, 0, 0, TO_ROOM);
			act("$n slowly fades out of existence..", TRUE, ch, 0, 0, TO_ROOM);
			char_from_room(ch);
			char_to_room(ch, real_room(3306), 0);
			act("$n slowly fades into existence, looking extremely annoyed.", TRUE, ch, 0, 0, TO_ROOM);
			act("$n sits behind his desk, and closes his eyes for a quick nap.", TRUE, ch, 0, 0, TO_ROOM);
		...
		if ((move == FALSE) && (GET_STAT(ch) == STAT_SLEEPING) && (ch->in_room == real_room(3306)))
		{
			act("$n snores loudly enough to shake the walls.", TRUE, ch, 0, 0, TO_ROOM);
			move = FALSE;
		}
	} /* end (!move) if loop */
	...
	if ((cmd != 0) && pl)
	{
		...
		switch (cmd)
		{
			...
			case CMD_ROSE:
				...
				{
					act("$n whispers to you, 'You're a fine lass if ever I did see one!'", FALSE, pl, 0, ch, TO_CHAR);
				...
				{
					act("$N tells you, 'Now, now. None of that mushy stuff with me, pal.'", FALSE, pl, 0, ch, TO_CHAR);
					act("$N tells $n, 'None of the mushy stuff with me, pal.'", TRUE, pl, 0, ch, TO_ROOM);
				}
				return TRUE;
				break;
			case CMD_LAUGH:
			...
			case CMD_TRIP:
				if ((c_obj != ch) || (!IS_AWAKE(ch)) || (IS_FIGHTING(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N snarls at you, 'Watch it, bub, or I just might have to teach you a lesson!'", FALSE, pl, 0, ch, TO_CHAR);
				act("$N snarls at $n, 'Watch it, bub, or I just might have to teach you a lesson!'", TRUE, pl, 0, ch, TO_ROOM);
				return TRUE;
				break;
			case CMD_SCREAM:
			case CMD_BURP:
			...
			case CMD_BIRD:
				if ((c_obj != ch) || (!IS_AWAKE(ch)) || (IS_FIGHTING(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N grumbles at you, 'Every damn place you go, someone has to be a clown.'", FALSE, pl, 0, ch, TO_CHAR);
				act("$N grumbles at $n, 'Every damn place you go, someone has to be a clown.'", TRUE, pl, 0, ch, TO_ROOM);
				return TRUE;
				break;
			case CMD_GRIN:
			case CMD_HICCUP:
			...
			case CMD_HI5:
				if ((c_obj != ch) || (!IS_AWAKE(ch)) || (IS_FIGHTING(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N giggles at you, finding that amusing.", FALSE, pl, 0, ch, TO_CHAR);
				act("$N giggles at $n, finding that amusing.", TRUE, pl, 0, ch, TO_ROOM);
				return TRUE;
				break;
			default:
				return FALSE;
	...
	switch (piergeiron_path[piergeiron_index])
	{
		...
		case 'W':
			...
			act("$n says, 'Time to take care of business!'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'S':
			ch->only.npc->default_pos = STAT_SLEEPING + POS_SITTING;
			SET_POS(ch, POS_SITTING + STAT_SLEEPING);
			act("$n lies down and quickly falls asleep behind his desk.", FALSE, ch, 0, 0, TO_ROOM);
			move = FALSE;
			break;

		case 'a':
			act("$n steps in front of the watchman and salutes.", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'Report, watchman!'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'b':
			act("$n steps in front of the guard and salutes.", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'Report, guard. What's new this day?'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'c':
			act("$n says, 'Greetings, friends!  Another fine day in Verzanan.'", FALSE, ch, 0, 0, TO_ROOM);
		...
		case 'e':
			act("$n says, 'Looks like another busy day in the port!  Keep the people in order, men.'", FALSE, ch, 0, 0, TO_ROOM);
			act("The guards salute, and nod in respect.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'E':
			act("$n smiles warmly at Kalara.", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'My greetings, m'lady. How's business?'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'g':
			act("Kalara says, 'Same as always, old man, and you come by anyway.'", FALSE, ch, 0, 0, TO_ROOM);
			act("Kalara chuckles politely.", FALSE, ch, 0, 0, TO_ROOM);
			act("Kalara pokes Lord Piergeiron in the ribs.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'h':
			act("$n says, 'Yeah, yeah. Hey, it's my job, y'know.'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'I'm off to find Khelben. Take care, Kalara.'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n bows deeply.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'i':
			act("The watchman says, 'Nothing happening here at the bazaar, sir.'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'Keep up the good work.'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'j':
			act("The watchman salutes.", FALSE, ch, 0, 0, TO_ROOM);
			act("The watchman says, 'Nothing unusual sir, same as always.'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'k':
			act("$n says, 'Anyone here seen Khelben?  Damn slippery mages are impossible to find..'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n sighs loudly.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'l':
			act("$n says with good cheer, 'Have a good one, boys!'", FALSE, ch, 0, 0, TO_ROOM);
			act("The men cheer and raise their mugs.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'm':
			act("The waiter says, 'Your usual sir?'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'You got it, old man. Need it to wake up.'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'n':
			act("The waiter pours a shot of whiskey. ", FALSE, ch, 0, 0, TO_ROOM);
			act("The waiter gives the shot to the tired Lord of Verzanan.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'o':
			act("$n swallows the shot in a single gulp, and hands back the glass.", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'Ahhhh, that hit the spot.'", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'p':
			act("$n says, 'Well, back to work, boys. See ya later on.'", FALSE, ch, 0, 0, TO_ROOM);
		...
		case 'A':
			act("$n says, 'The east gate of Verzanan is now closed!'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'You may reopen the gates at sunrise, men.'", FALSE, ch, 0, 0, TO_ROOM);
			act("The guards nod in acknowledgment.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'B':
			act("$n says, 'The north gate of Verzanan is now closed!'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'You may reopen the gates at sunrise, men.'", FALSE, ch, 0, 0, TO_ROOM);
			act("The guards nod in acknowledgment.", FALSE, ch, 0, 0, TO_ROOM);
			break;

		case 'F':
			act("$n says, 'The west gate of Verzanan is now closed!'", FALSE, ch, 0, 0, TO_ROOM);
			act("$n says, 'You may reopen the gates at sunrise, men.'", FALSE, ch, 0, 0, TO_ROOM);
			act("The guards nod in acknowledgment.", FALSE, ch, 0, 0, TO_ROOM);
			break;
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (number(1, 20))
			{
				...
				case 2:
					...
					act("$n peers around with a hungry look on his face.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n smiles at you with a warm, fatherly look.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					...
					break;
				case 5:
					act("$n seems distracted for a moment, as if pondering something.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 6:
					act("$n adjusts his uniform to look more authoritative.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 7:
					mobsay(ch, "I'll bet you anything Fafhrd has spies watching my every move.");
					mobsay(ch, "And hides when I'm around so I won't find him.");
					act("$n peers around.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 8:
					act("$n trips over a small stone, but recovers gracefully.", TRUE, ch, 0, 0, TO_ROOM);
				...
				case 10:
					...
					act("$n ponders the thought with an evil twinkle in his eye.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 11:
					act("$n considers you casually for a moment.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 12:
					act("$n smirks arrogantly, obviously proud of his station.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 13:
				...
				case 15:
					...
					break;
				case 16:
					mobsay(ch, "Well, well, well. You plan on staying out of trouble today?");
					break;
				case 17:
					mobsay(ch, "Hey look alive there!");
					act("$n rubs his chin while looking you over.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 18:
				...
				case 20:
					...
					act("$n stares in the sky, covering his eyes.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int piergeiron_guard(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			...
			case 2:
				...
				break;
			case 3:
				strcpy(buf, "An outlaw has attacked me! Come help lay this villain low, brethren!");
				do_yell(ch, buf, 0);
				break;
			case 4:
				/*
				...
				{
					act("$n performs a martial arts maneuver with blinding speed!", TRUE, ch, 0, 0, TO_ROOM);
	...
	if ((MIN_POS(ch, POS_STANDING + STAT_NORMAL)) && !number(0, 4))
	{
		...
		{
			case 2:
				act("$n looks around cautiously, watching for any possible threat to the lord.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				act("$n looks at you for a moment.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n keeps his hand on his sword hilt while near you.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
...
int wanderer(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 2:
					act("$n examines the animal tracks on the ground.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					mobsay(ch, "God, I love the outdoors!");
					break;
				case 4:
					act("$n looks at you with a curious expression.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 5:
					act("$n gazes off onto the horizon, looking for something.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int dog_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 2:
					act("$n whines as if looking for a handout.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n hears something in the distance and barks.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n wanders around, sniffing the ground.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 4:
					...
					break;
				case 5:
					act("$n whirls, listening intently for some unheard sound.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int drunk_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n stumbles and nearly falls, lost in his drunken stupor.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
				...
				case 3:
					...
					break;
				case 4:
					act("$n turns green and nearly hurls, but amazingly recovers.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int drunk_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n begins singing loudly, though his awful tone makes you cringe.", TRUE, ch, 0, 0, TO_ROOM);
					act("Dogs can be heard howling in the distance.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "OOoohhh! Loookie what weee have  here, a worthless ball offf horse manuure..");
					break;
				case 3:
					act("$n points at you and laughs uncontrollably for several minutes..", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n flips you the bird and mumbles something incoherent under his breath.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int drunk_three(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n shouts annoyingly, 'Where is that damn bartender!'", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					...
					break;
				case 3:
					act("$n loses his balance and falls to the ground, cursing all the while.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n stares off into space, seemingly lost in some mindless thought.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int homeless_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n holds out his hands, begging for food.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "Could ya spare a few coins? Just a few? I gots nuttin' ta eat tonight..");
					act("$n whimpers quietly.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n is overcome with a fit of coughing. He doesn't look well.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
...
int cat_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n approaches and bumps your leg, looking for attention.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					act("$n scratches at an itch.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n dives at something on the ground, playing.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks at you and mews, purring for attention.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				case 5:
					...
					break;
				case 2:
					mobsay(ch, "You wouldn't happen to know where the bazaar is, would you?");
					break;
				case 3:
					act("$n looks condescendingly at you, as if you're less than scum.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks you up and down, probably sizing up whether or not you're worth the effort.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int merchant_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	{
		if ((MIN_POS(ch, POS_STANDING + STAT_NORMAL)) && (ch->in_room == real_room(5400)))
		{
			switch (dice(2, 5))
			{
				case 5:
					act("$n looks impatient, as if he's waited years for his ship to come in.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "GOD, where is that blasted ship!");
					break;
				case 3:
					act("$n stares out the door, scanning the harbor for his ship.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					mobsay(ch, "Receptionist! Get that damn ship here! I've been waiting forever!");
					break;
				default:
					break;
			}
...
int farmer_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n looks a bit timid in this huge city.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks around for something to clean.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks out the window at the glorious city.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int baker_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n runs around the room, playing wildly.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "Hey, ma! Can we go outside and play?");
					break;
				case 3:
					act("$n crashes into a table while running around.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks around for something to play with.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n stares blankly into space, contemplating something.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
...
int cleric_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n sings a hymn in praise to the Gods. It is quite beautiful.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
				...
				case 3:
					...
					break;
				case 4:
					act("$n performs a magical gesture of some kind.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				case 5:
					...
					break;
				case 2:
					act("$n takes a long, deep breath as a cool breeze blows by.", TRUE, ch, 0, 0, TO_ROOM);
					mobsay(ch, "Hell of a day, isn't it..");
					break;
				case 3:
					act("$n checks the readiness of the catapult.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					mobsay(ch, "You should consider a career in the navy, strong as you are.");
					break;
				default:
					break;
			}
...
int warrior_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n shadow boxes, showing off his battle prowess.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "Don't you wish you were as strong and mighty as I?");
					break;
				case 3:
					act("$n sizes you up, as if considering your battle capabilities.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n screws up a sword maneuver, blushing furiously.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n keeps his hand on the hilt of his weapon while near you.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n stops suddenly as if having heard something odd.", TRUE, ch, 0, 0, TO_ROOM);
					act("After a few moments, $n continues on his way.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				case 5:
					...
					break;
				case 2:
					mobsay(ch, "Get lost, kid, or I might decide to relieve you of your pathetic existence.");
					break;
				case 3:
					act("$n growls as you, resembling a not-so-trained Doberman.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n lets off a roaring belch that echoes around the room.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
...
int casino_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 2:
					act("$n moves some gambling chips around so fast you almost can't follow his movements.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n shuffles the cards with the ease of a skilled pro.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					mobsay(ch, "Dealer raises 20.");
					break;
				case 5:
					act("$n deals out a card to one of the gamblers.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 6:
					mobsay(ch, "Feel lucky tonight, boys?");
					break;
				case 7:
					act("$n makes a perfect poker face, looking as rigid as a board..", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int casino_three(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (ch->in_room != real_room(GET_HOME(ch)))
	{
		...
		/*
		   Muahahahah
		 */
		act("$n looks around dazedily, and says 'Where the HELL am I?'", FALSE, ch, 0, 0, TO_ROOM);
		act("$n says 'Damn!  Luna'll KILL me, I gotta get back to work!'", FALSE, ch, 0, 0, TO_ROOM);
	...
	for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room)
	{
		...
		{
			if ((GET_ALIGNMENT(tch) < max_evil) && (IS_NPC(tch) || IS_NPC(GET_OPPONENT(tch))))
			{
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		...
		act("$n yells, 'HEY! No fighting in here, dead beat!'", FALSE, ch, 0, evil, TO_ROOM);
		act("The bouncer picks you and tosses you out of the bar!", FALSE, evil, 0, ch, TO_CHAR);
		...
		act("$N is thrown from the tavern, landing in a heap on the ground!", FALSE, ch, 0, evil, TO_NOTVICT);
		act("The bouncer snarls at you, 'Next time I'll break your neck, punk!'", FALSE, evil, 0, ch, TO_CHAR);
		act("The bouncer snarls at $N, 'Next time I'll break your neck, punk!'", FALSE, ch, 0, evil, TO_NOTVICT);
...
int casino_four(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (IS_FIGHTING(ch))
		{
			...
			{
				case 5:
					act("$n growls angrily, a look of rage on his face.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
				...
				case 3:
					...
					break;
				case 4:
					act("$n laughs at you as he viciously attacks.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks around for something to fight with.'", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
...
int guard_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 6:
				act("$n drinks deep from his bottle, and then lets out a roaring belch.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "How's about some entertainment, bartender! Where's that dancer.");
			...
			case 3:
				...
				break;
			case 4:
				act("$n says, 'Hey, bartender! Bring me another, dammit.'", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n laughs heartily, nearly falling off the barstool.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		mobsay(ch, "Heeey!  You c-can't do that, I'ma..I'ma a guard of Wateeerdeepp!");
		act("$n jumps off the stool and joins in the fight with a drunken grin.", FALSE, ch, 0, 0, TO_ROOM);
		...
		if (GET_STAT(ch) == STAT_SLEEPING)
		{
			switch (dice(2, 4))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n twitches in his sleep, probably having a dream.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int park_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 4:
					act("$n quacks loudly, running around in a circle.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					act("$n pecks at it's fur, cleaning itself.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 4))
			{
				case 4:
					...
					break;
				case 2:
					act("$n runs along the ground looking for worms.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
...
int park_four(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 4:
					act("$n stands on its back legs to get a better look at you.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
...
int park_five(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 4:
					act("$n's tail pops up as it considers you.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
...
int park_six(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 4:
					act("$n runs along the ground, looking terrified.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					act("$n digs a small nut from the grass and begins eating it.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n wriggles its little tail while looking at you.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks at you with eyes both angry and hateful.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n spits at the ground in front of you.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int tailor_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL) && !number(0, 2))
		{
			...
			{
				case 5:
					act("$n sorts through his many measuring tapes.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "Hello. You don't look like a cityguard, are you here for a fitting?");
					break;
				case 3:
					act("$n starts picking up small pieces of lint and thread.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks at you and says, 'You could stand to loose a few pounds.'", TRUE, ch, 0, 0, TO_ROOM);
...
int shopper_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n browses through the goods for sale here.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					...
					break;
				case 3:
					act("$n looks around frustrated, as if he can't find what he wants to buy.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n says, 'You can never find what you want in this damn bazaar!", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int shopper_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n smiles at you and says, 'Good day.'", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
				...
				case 3:
					...
					break;
				case 4:
					act("$n browses through the items for sale.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int rogue_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (IS_FIGHTING(ch))
		{
			...
			{
				case 5:
					act("$n says, 'You will DIE for intruding in my home!'", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 7))
			{
				...
				case 4:
					...
					break;
				case 5:
					act("$n looks around for an interesting prospect.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 6:
	...
	if (cmd != 0)
	{
		...
		if (IS_AGG_CMD(cmd))
		{
			if ((c_obj != ch) || (!IS_AWAKE(ch)))
				return FALSE;
			act("$N screams in terror at you, 'Help, help! Someone get this creep off of me!'", FALSE, pl, 0, ch, TO_CHAR);
			act("$N screams, 'Help, help! Someone get this creep off of me!'", TRUE, pl, 0, ch, TO_ROOM);
			if (ch->following)
				stop_follower(ch);
			return FALSE;
		}
		switch (cmd)
		{
			...
			case CMD_OGLE:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N whispers to you, 'I'm yours for 5 gold, sweetie pie.'", FALSE, pl, 0, ch, TO_CHAR);
				act("$N whispers something to $n.", TRUE, pl, 0, ch, TO_ROOM);
				return TRUE;
				break;
			case CMD_SMILE:
			case CMD_WINK:
				if (!IS_AWAKE(ch))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$n smiles evilly, looking for some fun..", TRUE, ch, 0, 0, TO_ROOM);
				return TRUE;
				break;
			case CMD_PUKE:
			case CMD_INSULT:
			...
			case CMD_PINCH:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N whispers to you, 'I like that.' in a sexy voice.", FALSE, pl, 0, ch, TO_CHAR);
				act("$N whispers something to $n in a sexy voice.", TRUE, pl, 0, ch, TO_ROOM);
				return TRUE;
				break;
			case CMD_WAKE:
				if (c_obj != ch)
				...
				{
					act("Your gentle nudging awakens $N, who yawns and clears her throat.", FALSE, pl, 0, ch, TO_CHAR);
...
int assassin_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n makes a lightning-fast move as he practices his backstab.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					...
					break;
				case 3:
					act("$n does a quick dodge in front of you, showing off his skill.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n watches you intently, a devious look in his eye.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				case 5:
					...
					break;
				case 2:
					mobsay(ch, "I.. I.. I-I looove fishhhing..  I-It's sooooo relaxing, y'know?");
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks across the dock for something or someone.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks as though he's been working hard all day.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n inspects the underside of the ship for flaws.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
...
int naval_three(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 7))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks out the window, across the harbor.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
		...
		else if (IS_FIGHTING(ch))
		{
			...
			{
				case 1:
					act("$n glares at you, putting more weight into her swing.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "You have met your match, ignorant fool!");
					break;
				case 3:
					act("$n dances and twists about, showing amazing grace in combat.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					/*
					...
					for (vict = world[ch->in_room].people; vict; vict = temp)
					{
						...
						{
							act("$n makes a powerful magical gesture.", TRUE, ch, 0, 0, TO_ROOM);
							act("A bolt of pure energy streaks out from her fingertips!", TRUE, ch, 0, 0, TO_ROOM);
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					mobsay(ch, "Just make sure you're not on the gates when I open them.");
					break;
				case 4:
					act("$n looks around the harbor, taking in everything.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int lighthouse_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((lighthouseone == 1) && (MIN_POS(ch, POS_STANDING + STAT_NORMAL)))
		...
		{
			case 0:
				act("$n stares long across ocean, taking a deep breath.", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 1:
				act("$n says, 'Beautiful view, isn't it?", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				act("$n says, 'Been workin' here 30 years and never got over how lovely it is.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				act("$n smiles at you with warm eyes.", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n says, 'Only thing needs doin' now is to rebuild this old place.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n says, 'Been here for a good hundred years, I reckin.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 6:
				act("$n says, 'Hehe.  Sometimes I'm surprised it's still standin.'", FALSE, ch, 0, 0, TO_ROOM);
				act("$n giggles softly.", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 7:
				act("$n says, 'This one used to be just fine, till they dug it all out.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 8:
				act("$n grumbles, 'God knows what they were doin' down there..'", FALSE, ch, 0, 0, TO_ROOM);
				act("$n chuckles.", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 9:
				act("$n says, 'Course the military never tells us common folk what's really goin' on.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 10:
				act("$n says, 'Aahhhh well, 'tis just the same anyway.", FALSE, ch, 0, 0, TO_ROOM);
				act("$n says, 'If they don't wanna tell us, it probably isn't important.", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 11:
				act("$n says, 'You hear about the pirate traffic?'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 12:
				act("$n says, 'I heard it's gettin' worse, just what we need.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 13:
				act("$n snarls, 'More jackass brigands to deal with.'", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 14:
				act("$n says, 'Ahh well, I'll shut up and enjoy the view.'", FALSE, ch, 0, 0, TO_ROOM);
			...
			case 24:
				break;
			case 25:
				act("$n stares long across ocean, looking for ships.", FALSE, ch, 0, 0, TO_ROOM);
				break;
			case 26:
			case 27:
			case 28:
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n scans the horizon for sea going vessels.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks at you curiously.", TRUE, ch, 0, 0, TO_ROOM);
					break;
		...
		if (IS_FIGHTING(ch))
		{
			switch (dice(1, 4))
			{
				...
				case 3:
					...
					break;
				case 4:
					act("$n slaps you with a lightning fast move.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int seabird_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n flies close by, looking for a handout.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				case 5:
					...
					break;
				case 2:
					act("$n notices something on the ground, and stares at it intently.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
					act("$n flaps it's wings about on the ground.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n stands absolutely still, as if trying to look stoic.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (dice(2, 5))
			{
				...
				case 3:
					...
					break;
				case 4:
					mobsay(ch, "Have you seen a small boy, around eight years old, with red hair?");
		...
		if (IS_FIGHTING(ch))
		{
			switch (dice(1, 4))
			{
				case 1:
					...
					break;
				case 2:
					mobsay(ch, "Don't hurt me, please! I have three children to care for!");
					break;
				case 3:
...
int commoner_three(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n gazes long across the ocean, lost in thought.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					...
					break;
				case 3:
					act("$n takes a deep breath as the breeze blows in, looking very relaxed.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n closes her eyes, and looks deep in thought.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
...
int commoner_four(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n grunts as he tries a difficult move.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
...
int commoner_six(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 5:
					act("$n hoots with joy as the struggle continues.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 2:
					mobsay(ch, "Go, dad, go!");
					break;
...
int crier_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else
	{
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			...
			{
				case 2:
					act("$n gets ready for another bout of shouting.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 3:
				...
				case 4:
					...
					break;
				case 5:
					act("$n looks at you for a moment, seeing if his audience is listening.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 6:
					act("$n paces back and forth, apparently excited about his work.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 7:
					mobsay(ch, "City officials report that unknown tunnels have been discovered in the graveyard!");
					mobsay(ch, "Best to be careful where you step there.");
					break;
				case 8:
					mobsay(ch, "I hear Gelian has the best deal in town for precious gemstones!");
					break;
				case 9:
					act("$n says softly, 'Rumor has it Kalara and Lord Piergeiron are an item!'", TRUE, ch, 0, 0, TO_ROOM);
					act("$n giggles evilly.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 10:
					mobsay(ch, "Lord Piergeiron is looking for volunteers to join the city's militia!");
					mobsay(ch, "Mostly for when we get hit by goblin raids and whatnot.");
					break;
				case 11:
					strcpy(buf, "If you're headin' south to Calimport, beware!");
					do_yell(ch, buf, 0);
					strcpy(buf, "Trolls, Giants, and many dangers may threaten your lives, go with caution.");
					do_yell(ch, buf, 0);
					break;
				case 12:
					...
					break;
				case 13:
					mobsay(ch, "Rumor has it that tunnels have been dug out beneath the city!");
				...
				case 14:
					...
					break;
				case 15:
					strcpy(buf, "Lord Piergeiron is looking for brave adventurers to fight off the trolls!");
					do_yell(ch, buf, 0);
					strcpy(buf, "If you can help, form a group and head south.");
					do_yell(ch, buf, 0);
					break;
				case 16:
					mobsay(ch, "The druids expect a storm sometime soon, look out for it!");
					break;
				case 17:
					mobsay(ch, "Hey, folks, I hear there are good things for sale in the bazaar!");
					break;
				case 18:
					mobsay(ch, "Announcement! Jakar has made a new secret formula for his ribs!");
					mobsay(ch, "Last one there is starving pig!");
					break;
				case 19:
					mobsay(ch, "There is rumor that burglars are wandering the rooftops of southern Verzanan!");
					mobsay(ch, "Be careful up there, I hear they're mighty tough!");
					break;
				case 20:
					mobsay(ch, "I hear the call girls south of here will show you a good time for 500 coins.");
					act("$n winks knowingly.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 21:
					strcpy(buf, "Arena contests! All contenders report to the arena for a good fight!");
					do_yell(ch, buf, 0);
					act("$n cackles with anticipation of the blood feast.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 22:
					...
					break;
				case 23:
					mobsay(ch, "Remember to fill your barrels and ration up before leaving town!");
					mobsay(ch, "It's a hard and dangerous world out there.");
					break;
				case 24:
					act("$n says softly, 'I hear the old fisherman out on Deepwater Isle has discount tickets to the Moonshaes!'", TRUE, ch, 0, 0, TO_ROOM);
					act("The guard glares at the town crier.", TRUE, ch, 0, 0, TO_ROOM);
					act("$n blushes, and backs off.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 25:
					mobsay(ch, "Rumor has it that there is woodsman east of Verzanan who has a tale to tell.");
				...
				case 27:
					mobsay(ch, "Be Warned!");
					mobsay(ch, "The Brotherhood of the Black Fire reports the end of the world is at hand!");
					mobsay(ch, "Looks like they're predicting earlier this year..");
					break;
				case 28:
					mobsay(ch, "Lord Piergeiron is asking for the return of the jewels stolen from his home recently.");
					mobsay(ch, "He promises that no harsh action will be taken against the thief.");
					mobsay(ch, "Which translates to: You wont be burned at the stake instantly, he'll give you a week and then cook ya.");
					mobsay(ch, "Anyone foolish enough to turn em in, inquire at the jail house.");
					break;
				case 29:
					mobsay(ch, "Oh! And one important thing I have to tell..");
					act("$n is struck in the back of the head by an airborne tomato, though his assailant cannot be seen.", TRUE, ch, 0, 0, TO_ROOM);
					mobsay(ch, "HEY! That hurt!");
					break;
				case 30:
					mobsay(ch, "I hear the Tower of High Sorcery in the woods to the north is haunted.");
					mobsay(ch, "Many an adventurer is said to have entered, but few live to tell the tale.");
					break;
				case 31:
					...
					break;
				case 32:
					mobsay(ch, "Scouts have reported a strange finding while at sea last week.");
					mobsay(ch, "A large rectangular object was discovered standing on top of the water!");
					mobsay(ch, "It's nature was unknown, though it was definitely magical.");
					break;
				case 33:
				...
				case 35:
					...
					break;
				case 36:
					mobsay(ch, "Oooo, I haven't seen someone as ugly as you in a LONG time!");
					act("$n falls down laughing.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 37:
					mobsay(ch, "The sages of Verzanan have sometimes spoken of travelers who wander the land with great knowledge of the world..");
					mobsay(ch, "Some are great wizards who can teach rare and strange magic, while others know the way to hidden treasures.");
					mobsay(ch, "Seek them out, for they carry ancient wisdom.");
					break;
				case 38:
					mobsay(ch, "Oh! Lord Piergeiron wanted me to tell you.");
					mobsay(ch, "Make sure you give donations to the soup kitchen south of town, the poor need your help.");
					break;
				case 39:
					...
					do_yell(ch, buf, 0);
					send_to_zone_outdoor(12, "A housewife screams, 'Shut up, you bloody idiot!\r\n'");
					break;
				case 40:
					act("$n drinks some water to clear his throat.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 41:
					mobsay(ch, "The Gods have brought good weather to the farmers again this year!");
					mobsay(ch, "They must be smiling upon us.");
					break;
				case 42:
					act("A boy runs through the room, hitting the crier and knocking him over.", TRUE, ch, 0, 0, TO_ROOM);
					act("A boy says, 'Shut your stupid hole, fool!', and runs off", TRUE, ch, 0, 0, TO_ROOM);
					act("$n screams, 'Hey!  Stop that little shit!'", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					break;
			}
		...
		{
			crierhour2 = 1;
			strcpy(buf, "The ship heading to the Moonshaes leaves in three hours, folks!");
			do_yell(ch, buf, 0);
		}
		if ((time_info.hour == 10) && (crierhour2 == 0))
		{
			crierhour2 = 1;
			strcpy(buf, "The ship heading for Calimport leaves in two hours, folks!");
			do_yell(ch, buf, 0);
		}
	...
	if (IS_FIGHTING(ch))
	{
		...
		do_yell(ch, buf, 0);
		send_to_zone_outdoor(world[ch->in_room].zone, "People all over the city can be heard cheering.\r\n");
	}
	return (FALSE);
}
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL) && !number(0, 2))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks at you intently for a moment, then smiles.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n looks around, observing everything for trouble.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				default:
					return FALSE;
					break;
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL) && !number(0, 2))
		{
			switch (dice(2, 5))
			{
				...
				case 2:
					...
					break;
				case 3:
					act("$n looks you up and down for a moment, then goes back to his duties.", TRUE, ch, 0, 0, TO_ROOM);
					break;
				case 4:
					act("$n scans the area for signs of trouble.", TRUE, ch, 0, 0, TO_ROOM);
					break;
		...
		switch (cmd)
		{
			...
			case CMD_HI5:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N vomits all over $n, giving him something to frown about..", TRUE, ch, 0, 0, TO_ROOM);
				act("$N vomits all over your clothing, giving you something to frown about.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_INSULT:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
			...
			case CMD_FLIRT:
				...
				act("$N spits and coughs in disgust saying, 'Bleh!'.", TRUE, pl, 0, ch, TO_ROOM);
				act("$N spits and coughs in disgust saying, 'Bleh!'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_PUKE:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N sees $n puke, and begins vomiting uncontrollably.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N sees you puke, and begins vomiting uncontrollably.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_CACKLE:
			case CMD_SCREAM:
			...
			case CMD_SING:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N says, 'Will ya keep it down, $n? My head hurts!'.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N says, 'Will ya keep it down? My head hurts!'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_SNIFF:
			case CMD_WHINE:
			...
			case CMD_HISS:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N says, 'Quit cryin' about it, we're all in this together.'", TRUE, ch, 0, 0, TO_ROOM);
				act("$N says, 'Quit cryin about it, we're all in this together.'", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_BURP:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N applauds and says with great cheer, 'Worthy! Good one!'.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N applauds and says with great cheer, 'Worthy! Good one!", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_FART:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N says, 'Oh, god..  Will ya do that outside?!?'.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N says, 'Oh, god..  Will ya do that outside?!?'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_SLAP:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N slaps $n back with great cheer, thinking it all a game.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N slaps you back with great cheer, thinking it all a game.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_LAUGH:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N frowns and says, 'What are YOU laughin' about, $n?'.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N frowns and says, 'What are YOU laughin' about, bub?'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_HUG:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N says, 'Don't hug me, $n, ya smell awful!'.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N says, 'Don't hug me jailbait, ya smell awful!'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_GRIN:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
				...
				act("$n grins and whispers to you, 'Got a plan mate?'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			case CMD_GASP:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
			...
			case CMD_POUT:
				if ((c_obj != ch) || (!IS_AWAKE(ch)))
					return FALSE;
				do_action(pl, Gbuf1, cmd);
				act("$N says, 'Quit yer pouting, $n, and think of a way outta here'.", TRUE, ch, 0, 0, TO_ROOM);
				act("$N says, 'Quit yer poutin' and think of a way outta here'.", FALSE, pl, 0, ch, TO_CHAR);
				return TRUE;
				break;
			default:
				return FALSE;
		...
		if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
		{
			switch (number(2, 14))
			{
				...
				case 7:
					...
					break;
				case 8:
					mobsay(ch, "Hey, jailer! Gimme some ale, I'm dyin' a thirst in here!");
					break;
				case 9:
...
int guild_protection(P_char ch, P_char pl)
{
	...
	if (!SanityCheck(ch, "guild_protection - guard") || !SanityCheck(pl, "guild_protection - vict") || !IS_FIGHTING(ch) || (GET_OPPONENT(ch) != pl))
		return 0;

	if (IS_NPC(pl))
		return 0;

	mobsay(ch, "Begone from here Outlaw! None are allowed to attack guild guardians!");
	act("$n pressed a small metal pin on $s chest, which flares with brilliant blue light!", FALSE, ch, 0, 0, TO_ROOM);
	send_to_char("&+WYou feel a wretching pain penetrate you and drain your lifefore away!\r\n&+WOuch! That really did HURT!\r\n", pl);
...
int guild_guard_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5500)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The holy warrior steps in front of $n, blocking $s entry into the guild.", FALSE, pl, 0, 0, TO_ROOM);
			act("The holy paladin says, 'I'm sorry, but only holy paladins may enter here.'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The holy warrior steps in front of you, blocking your way.\r\n", pl);
			send_to_char("The holy paladin says, 'I'm sorry, but only holy paladins may enter here.'\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The holy warrior bows reverently before $n as $s enters.", FALSE, pl, 0, 0, TO_ROOM);
			act("The holy paladin tells $n, 'You are welcome, brethren..'", FALSE, pl, 0, 0, TO_ROOM);
			act("$N leaves east into the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			send_to_char("The holy warrior bows reverently before you as you enter.\r\n", pl);
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("The holy warriors change positions at their post.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n performs a dazzling display of swordwork against you.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				mobsay(ch, "This fight is pointless! Surrender now before I am forced to kill you!");
				break;
			default:
				break;
		}
...
int guild_guard_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5510)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The minotaur slaps $n across the face as $s tries to enter the guild.", FALSE, pl, 0, 0, TO_ROOM);
			act("The minotaur snarls, 'Get lost, idiot, or I'll have you for lunch!'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The huge minotaur slaps you across the face! *SMACK*\r\n", pl);
			send_to_char("The minotaur snarls, 'Get lost, idiot, or I'll have you for lunch!'\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The minotaur fixes $n with a stern eye as $s enters the guild.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The minotaur fixes you with a stern eye as you enter the guild.\r\n", pl);
			act("$N leaves east, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the west.\r\n", (GET_NAME(pl)));
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n makes a face, looking incredibly bored.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n looks at you with a hungry glint in his eye.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n grows menacingly and foams at the mouth.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "Die, worm!! I'm hungry, and you'll make a PERFECT lunchtime snack!");
				break;
			case 3:
				act("$n charges at you!", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				mobsay(ch, "Ha! Come now, child, surely you can fight better than THAT!");
...
int guild_guard_three(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5520)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The monk respectfully blocks $n's entry into the temple, and whisper something.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The monk respectfully blocks your entry into the temple.\r\n", pl);
			send_to_char("The monk whispers to you, 'Only pure warriors may enter here..'\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The monk bows respectfully before $n as $s enters.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The monk bows before you as you enter.\r\n", pl);
			act("$N leaves south, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n does a flip to the side, and attack from a different angle.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "The fight is pointless! You cannot win!");
				break;
			case 3:
				act("$n moves with blinding speed as he attacks.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int guild_guard_four(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5534)) && (cmd == CMD_UP))
	{
		...
		{
			act("The bouncer roughly grabs $n and pushes $s back.", FALSE, pl, 0, 0, TO_ROOM);
			act("The bouncer says, 'You don't have any business up there, pal, get lost.'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The bouncer roughly grabs you and pushes you back.\r\n", pl);
			send_to_char("The bouncer says, 'You don't have any business up there, pal, get lost.\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The bouncer steps aside as $n climbs up the staircase.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The bouncer steps aside as you climb up the staircase.\r\n", pl);
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		...
		StopAllAttackers(evil);
		act("$n yells, 'HEY! No fighting in here, dead beat!'", FALSE, ch, 0, evil, TO_ROOM);
		act("The bouncer picks you and drags you out of the bar!", FALSE, evil, 0, ch, TO_CHAR);
		act("$n drags $n out towards the street!", FALSE, ch, 0, evil, TO_NOTVICT);
		char_from_room(evil);
		char_to_room(evil, real_room(5533), 0);
		char_from_room(ch);
		char_to_room(ch, real_room(5533), 0);
		act("A bouncer storms into the room dragging $N by the collar", FALSE, ch, 0, evil, TO_NOTVICT);
		act("$N kicks and screams, all to no avail, as the bouncer drags $S east.", FALSE, ch, 0, evil, TO_NOTVICT);
		char_from_room(evil);
		...
		act("$N is thrown from the tavern and lands in a heap on the ground!", FALSE, ch, 0, evil, TO_NOTVICT);
		act("The bouncer snarls at you, 'Next time I'll break your neck, punk!'", FALSE, evil, 0, ch, TO_CHAR);
		act("The bouncer snarls at $N, 'Next time I'll break your neck, punk!'", FALSE, ch, 0, evil, TO_NOTVICT);
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n flexes his muscles, trying to look superior.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				...
				break;
			case 4:
				act("$n stares you down, thinking himself to be hot shit.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
...
int guild_guard_five(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5540)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The unicorn walks in front of $n, preventing $s from going south.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The unicorn walks in front of you, preventing you from going south.\r\n", pl);
			send_to_char("You hear a voice in your head, 'I'm sorry, but you cannot enter here, friend..'\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The unicorn nudges $n affectionately as $s enters the guild.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The unicorn nudges you affectionately as you enter the guild.\r\n", pl);
			act("$N leaves south, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the north.\r\n", (GET_NAME(pl)));
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n nibbles some grass near one of the trees.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
			...
			case 4:
				...
				break;
			case 5:
				act("$n walks in and about the trail, grazing.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("You hear a voice in your head, 'Stop this madness!'", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				act("$n forces a suggestion into your mind, 'FLEE! RUN AWAY!'", TRUE, ch, 0, 0, TO_ROOM);
				send_to_char("You flee in panic, running wildly under the unicorn's spell.", pl);
				do_flee(pl, 0, 2);
				send_to_char("You flee in panic, running wildly under the unicorn's spell.", pl);
				do_flee(pl, 0, 2);
				send_to_char("You flee in panic, running wildly under the unicorn's spell.", pl);
				...
				break;
			case 3:
				act("$n moves with blinding speed as it attacks.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
...
int guild_guard_six(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5560)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The massive treant blocks $n from going east with a wall of branches.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The massive treant blocks you from going east with a wall of branches.\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The massive treant drops a leaf on $n as $s enters the grove.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("A leaf falls on your head from the treant above as you enter the grove.\r\n", pl);
			act("$N leaves east, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the west.\r\n", (GET_NAME(pl)));
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("You can hear leaves rustling in the branches of the trees above.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				act("You feel as if the massive treant is watching you.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n strikes at you with dozens of branches.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("The massive treant issues a very low groan!", TRUE, ch, 0, 0, TO_ROOM);
				act("The sound blasts your ears painfully, causing you to scream and flee!", TRUE, ch, 0, 0, TO_ROOM);
				send_to_char("You flee in panic, running wildly under the treant's spell.", ch);
				do_flee(pl, 0, 2);
				send_to_char("You flee in panic, running wildly under the treant's spell.", ch);
				do_flee(pl, 0, 2);
				break;
...
int guild_guard_seven(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5570)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The flesh golem roughly grabs $n and tosses $s backwards.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The flesh golem roughly grabs you and tosses you backwards!\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The golem looks at $n for a moment as $s enters the dark grove.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The golem looks at you for a moment as you enter the dark grove.\r\n", pl);
			act("$N leaves east, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the west.\r\n", (GET_NAME(pl)));
...
int guild_guard_eight(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(5572)) && (cmd == CMD_EAST))
	{
		if (!GET_CLASS(pl, CLASS_NECROMANCER))
		{
			act("The guard steps in front of $n, preventing $s entrance into the tower.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guard steps in front of you, preventing your entrance into the tower.\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The guard bows deeply before $n, allowing $s entrance.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guard bow deeply before you, allowing you entrance.\r\n", pl);
			strcpy(tbuf, "door");
			do_open(ch, tbuf, 0);
			act("$N leaves east, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the west.\r\n", (GET_NAME(pl)));
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n looks around, as if searching for something.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n casts a minor cantrip, probably to amuse himself.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			case 1:
				...
				act("You slowly fade out, and rematerialize on Deepwater Island!", TRUE, ch, 0, 0, TO_ROOM);
...
int guild_guard_nine(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(3038)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The guard bows curtly before $n, and motions $s inside.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guard bows curtly before you, and motions you inside.\r\n", pl);
			strcpy(tbuf, "door");
			do_open(ch, tbuf, 0);
			act("$N leaves south, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the north.\r\n", (GET_NAME(pl)));
		...
		{
			act("The guard steps in front of $n, barring $s from entering the tower.", FALSE, pl, 0, 0, TO_ROOM);
			act("The guard tells $n, 'You may not enter here!'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guard steps in front of you, barring you from entering the tower.\r\n", pl);
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 4:
				...
				break;
			case 5:
				act("$n kicks a stone near the tower, looking very bored.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
...
int guild_guard_ten(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(3067)) && (cmd == CMD_NORTH))
	{
		...
		{
			act("The guild guardian smiles at $n, and allows $s inside.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guild guardian smiles at you, and allows you inside.\r\n", pl);
			strcpy(tbuf, "door");
			do_open(ch, tbuf, 0);
			act("$N leaves north, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
		...
		{
			act("The guild guardian stops $n before $s can enter the temple.", FALSE, pl, 0, 0, TO_ROOM);
			act("The guild guardian says, 'Only those of holy faith may enter.'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guild guardian stops you before you can enter the temple.\r\n", pl);
			send_to_char("The guild guardian says, 'Only those of holy faith may enter.'\r\n", pl);
			return TRUE;
		}
	}
	if (pl)
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n smiles warmly at you, and appears to be nice fellow.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n frowns at you, more out of pity than anger.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "You should not be doing this, it is pointless!");
				break;
			case 3:
				act("$n looks annoyed by your assault, but little more.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int guild_guard_eleven(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(3055)) && (cmd == CMD_SOUTH))
	{
		...
		{
			act("The guardian says to $n, 'Heya, mate, go right on in!", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guardian says to you, 'heya, mate, go right on in!\r\n", pl);
			strcpy(tbuf, "door");
			do_open(ch, tbuf, 0);
			act("$N leaves south, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the north.\r\n", (GET_NAME(pl)));
		...
		{
			act("The guardian steps in front of the door, barring $n's entrance.", FALSE, pl, 0, 0, TO_ROOM);
			act("The guardian says, 'Only warriors can enter here! Now, get lost..'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The guardian steps in front of the door, barring your entrance.\r\n", pl);
			send_to_char("The guardian says, 'only warriors can enter here! Now, get lost..'\r\n", pl);
			return TRUE;
		}
	}
	if (pl)
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n fights you in a perfect rhythm, dancing and dodging about.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "Boy! Yer not too bright, are ya! When you gonna give in and flee kid?");
				break;
			case 3:
				act("$n slaps you as he dances by, making you look like an idiot.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int guild_guard_twelve(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(3283)) && (cmd == CMD_EAST))
	{
		...
		{
			act("The rogue slaps $n across the face as $s tries to enter the guild.", FALSE, pl, 0, 0, TO_ROOM);
			act("The rogue snarls at $n, 'Get lost, kid.'", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The rogue slaps you across the face as you try and enter the guild.\r\n", pl);
			send_to_char("The rogue snarls at you, 'Get lost, kid.'\r\n", pl);
			return TRUE;
		}
		else
		{
			act("The rogue nods to $n, and motions $s to enter.", FALSE, pl, 0, 0, TO_ROOM);
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n seems to stick close to the back wall.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n whispers something to one of the patrons.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			case 1:
				...
				break;
			case 2:
				mobsay(ch, "You wanna die that bad, eh, fool? I'm only too happy to help!");
				break;
			case 3:
...
int guild_guard_thirteen(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if ((ch->in_room == real_room(2951)) && (cmd == CMD_NORTH))
	{
		...
		{
			act("The assassin trips $n as $s tries to go north.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The assassin trips you as you try to go north. *ouch*\r\n", pl);
			SET_POS(pl, POS_KNEELING + GET_STAT(pl));
			return TRUE;
		}
		else
		{
			act("The assassin bows, though never taking his eyes off $n as $s pass by him.", FALSE, pl, 0, 0, TO_ROOM);
			send_to_char("The assassin bows, though never taking his eyes off you as you pass by him.\r\n", pl);
			act("$N leaves north, entering the guild.", FALSE, ch, 0, pl, TO_NOTVICT);
			snprintf(Gbuf3, MAX_STRING_LENGTH, "%s arrives from the south.\r\n", (GET_NAME(pl)));
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n eyes you cautiously, waiting for some trick.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				mobsay(ch, "What's up?");
				break;
			case 4:
				act("$n leans against the wall, staring at you.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n absently flips a dagger through the air.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n cackles with glee as he attacks, obviously enjoying the fray..", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
				mobsay(ch, "I'll bet you've come in search of Kang, eh?");
				mobsay(ch, "Well, they're gonna have a harder time searching for your corpse!");
				break;
			case 3:
				act("$n moves with deadly grace, and is obviously very skilled.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int young_paladin_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 3:
				...
				break;
			case 4:
				act("$n practices a sword maneuver, loses his balance, and falls over.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n smiles at you.", TRUE, ch, 0, 0, TO_ROOM);
				break;
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n fights furiously, although his inexperience shows.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
...
int wrestler_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n circles around the arena, looking for a good attack opening.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				mobsay(ch, "You ain't no match fer me, dead beat!");
				break;
			case 4:
				act("$n dives at his opponent with a vengeance.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n grunts as his opponent gets the better of him.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n tries to pull a wrestling move on you.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 4:
				...
				break;
			case 5:
				act("$n tries to look impressive by practicing his backstab.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n chops at you with her hands in a wicked hook pattern.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int selune_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n slips and falls off his barstool, burping loudly.", TRUE, ch, 0, 0, TO_ROOM);
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n tries to flee and falls flat on his ass.", TRUE, ch, 0, 0, TO_ROOM);
...
int selune_two(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n scans the bar, as if looking for someone.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				mobsay(ch, "Hey, could you bring us another round?");
				break;
			case 4:
				act("$n smiles at you, then goes back to his conversation.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n takes a long draught from his bottle.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n swings, *SMACK* And hits you right across the jaw.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
...
int selune_four(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n stumbles and nearly falls on his ass.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
			...
			case 5:
				...
				break;
			case 6:
				act("$n tries to stumble into the other room, but fails.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 7:
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n falls on his ass as he tries to kill you.", TRUE, ch, 0, 0, TO_ROOM);
				SET_POS(ch, POS_SITTING + GET_STAT(ch));
				break;
			case 2:
...
int selune_five(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n moves slowly through the room, dancing all the way.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
				act("$n does a little dance in front of one of the patrons.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				act("$n smiles at you as she dances by.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 5:
				act("$n moves with the light-footed grace of a elven maiden.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		switch (dice(2, 5))
		{
			...
			case 4:
				...
				break;
			case 5:
				act("$n smiles and laughs at his friend's joke.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			default:
				break;
		}
	...
	else if (IS_FIGHTING(ch))
	{
		switch (dice(1, 4))
		{
			...
			case 2:
				...
				break;
			case 3:
				act("$n attempts to cast a spell at you, but it fails.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 4:
				mobsay(ch, "Someone help me!");
				break;
...
int young_necro_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	else if (MIN_POS(ch, POS_STANDING + STAT_NORMAL))
	{
		...
		{
			case 2:
				act("$n casts a minor cantrip, and smiles with pride.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 3:
	...
	else if (IS_FIGHTING(ch))
	{
		...
		{
			case 1:
				act("$n casts a spell, but it fizzles out and fails.", TRUE, ch, 0, 0, TO_ROOM);
				break;
			case 2:
...
int bouncer_one(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	const int b_path[] = {5533, 5531, 5530, 3258, -1};
	...
	if (evil && (GET_ALIGNMENT(GET_OPPONENT(evil)) >= 0))
	{
		while (evil && ch && (evil->in_room == ch->in_room) && (real_room(b_path[bp_var]) == ch->in_room))
		{
			switch (bp_var)
			{
				case 0:
					...
					StopAllAttackers(evil);
					act("$n yells, 'HEY! No fighting in here, dead beat!'", FALSE, ch, 0, evil, TO_ROOM);
					act("The bouncer picks you up and drags your ass out of the tavern!", FALSE, evil, 0, ch, TO_CHAR);
					act("$n grabs $N by the collar and drags $S out of the tavern!", FALSE, ch, 0, evil, TO_NOTVICT);
					break;
				case 1:
					act("A bouncer storms into the room, dragging $N by the collar!", FALSE, ch, 0, evil, TO_NOTVICT);
					act("$N kicks and flails around as $s is dragged out.", FALSE, ch, 0, evil, TO_NOTVICT);
					act("Many of the patrons look on in interest or laugh openly.", FALSE, ch, 0, evil, TO_NOTVICT);
					send_to_char("You are dragged through the tavern kicking and screaming.\r\n", ch);
				...
				case 2:
					...
					break;
				case 3:
					act("A bouncer throws $N from the tavern, who lands in a heap on the ground!", FALSE, ch, 0, evil, TO_NOTVICT);
					act("The bouncer throws you onto the ground. *ouch* You land in a heap.", FALSE, evil, 0, ch, TO_CHAR);
					act("The bouncer snarls at you, 'Next time I'll break your neck, punk!'", FALSE, evil, 0, ch, TO_CHAR);
					act("The bouncer snarls at $N, 'Next time I'll break your neck, punk!'", FALSE, ch, 0, evil, TO_NOTVICT);
					act("The bouncer reenters the Smile.", FALSE, ch, 0, evil, TO_NOTVICT);
			...
			else
			{
				...
				act("A bouncer walks back in, smiling smugly.", TRUE, ch, 0, 0, TO_ROOM);
				return (TRUE);
			}
		}
	}
...
int bouncer_two(P_char ch, P_char pl, int cmd, char *arg)
{
	P_char    tch, evil = NULL;
	int       bp_var = 0, max_evil = 1000;
	const int b_path[] = {5531, 5530, 3258, -1};
...
int bouncer_four(P_char ch, P_char pl, int cmd, char *arg)
{
	P_char    tch, evil = NULL;
	int       bp_var = 0, max_evil = 1000;
	const int b_path[] = {5532, 5531, 5530, 3258, -1};
