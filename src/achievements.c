...
void do_achievements(P_char ch, char *arg, int cmd)
{
	...
	if (affected_by_spell(ch, ACH_DO_YOU_MINE))
		...
		snprintf(buf3,
		         MAX_STRING_LENGTH,
		         "  %s            %s                             %s: &+W%d%%\r\n",
		         "&+LD&+co &+CY&+yo&+wu &+YM&+Wi&+mn&+Me&N",
		         "&+wMine 1000 ore&n",
		         "&+w15% increase in ore prices&n",
		         ore / 10);
	strcat(buf, buf3);
	//-----Miner
...
void update_achievements(P_char ch, P_char victim, int cmd, int ach)
{
	...
	{
		if (!affected_by_spell(ch, AIP_YOUSTRAHDME) && !affected_by_spell(ch, AIP_YOUSTRAHDME2) && !affected_by_spell(ch, ACH_YOUSTRAHDME))
		{
	...
	/*  FULL STATS  - gellz  */
	if ((!affected_by_spell(ch, ACH_DEATHSDOOR)) && (ch->base_stats.Str == 100) && (ch->base_stats.Agi == 100) && (ch->base_stats.Dex == 100) && (ch->base_stats.Con == 100) &&
	    (ch->base_stats.Pow == 100) && (ch->base_stats.Wis == 100) && (ch->base_stats.Int == 100) && (ch->base_stats.Cha == 100))
	{
	...
	for (findaf = ch->affected; findaf; findaf = next_af)
	{
		...
		// You Strahd Me2
		if ((findaf && findaf->type == AIP_YOUSTRAHDME) && (ach == 2) && IS_NPC(victim) && (GET_VNUM(victim) == VMOB_STRAHD_ACH_CHERNOVOG))
		{
			findaf->type = AIP_YOUSTRAHDME2;
		}

		// You Strahd Me - Finish
		if ((findaf && findaf->type == AIP_YOUSTRAHDME2) && (ach == 2) && IS_NPC(victim) && (GET_VNUM(victim) == VMOB_STRAHD_ACH_STRAHD))
		{
...
void update_addicted_to_blood(P_char ch, P_char victim)
{
	...
	if (IS_NPC(victim) && GET_LEVEL(victim) >= GET_LEVEL(ch) - 5 && !IS_PC_PET(victim) && !affected_by_spell(victim, TAG_CONJURED_PET))
	{
