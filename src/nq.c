...
extern void           act_convert(char *, const char *, P_char, P_char, P_obj, void *, int);
...
int nq_load_actor(struct nq_actor *actor, struct nq_quest *quest)
{
	...
	for (tch = world[room].people; tch; tch = tch->next_in_room)
		if (IS_NPC(tch) && !nq_acting(tch, quest) && GET_VNUM(tch) == vnum)
			break;
 ...
 */
int nq_find_actor(P_char mob, struct nq_quest **quest, struct nq_instance **instance, struct nq_actor **actor)
{
 ...
 */
void nq_reward_player(struct nq_action *action, struct nq_instance *instance, struct nq_quest *quest, P_char ch, P_char mob, bool kill)
{
	struct nq_item     *item;
	...
	{
		ch->only.pc->skills[action->reward->skill->skill].taught += action->reward->skill->value;
	...
	else
	{
		...
		{
			for (t_instance = quest->instance; t_instance->next != instance; t_instance = t_instance->next)
				;
 ...
 */
int nq_test_single_action(struct nq_action *action, struct nq_instance *instance, struct nq_quest *quest, P_char ch, P_char mob, char *phrase, bool kill)
{
	P_obj               obj;
	...
	if (((action->flag & NQ_ACT_DEATH) && !kill) || (!(action->flag & NQ_ACT_DEATH) && kill))
		return 0;
	...
	mob_cash = mob->points.cash[0] + 10 * mob->points.cash[1] + 100 * mob->points.cash[2] + 1000 * mob->points.cash[3];
...
int nq_action_check_all(P_char ch, P_char mob, char *phrase, bool kill)
{
	...
	{
		for (i = 0, t_instance = quest->instance; t_instance && !was_accepted; t_instance = t_instance->next)
		{
		...
		{
			if (nq_test_single_action(action, instance, quest, ch, mob, phrase, kill))
			{
				nq_reward_player(action, instance, quest, ch, mob, kill);
				return 1;
			}
		}
		else if ((ch->player.m_class & quest->allowed_classes) != 0 && quest->allowed_races[GET_RACE(ch)] != 0 && i < quest->allowed_instances &&
		         (!quest->skill || ch->only.pc->skills[quest->skill->skill].taught == quest->skill->value) && nq_test_single_action(action, instance, quest, ch, mob, phrase, kill))
		{
 ...
 */
void nq_char_death(P_char ch, P_char victim) { nq_action_check_all(ch, victim, NULL, 1); }
...
struct nq_quest *nq_parse_quest(char *fname)
{
	...
	while (node)
	{
		...
		{
			quest->actor_template[actor_templates++] = nq_parse_actor_template(node);
		}
 ...
 */
int nq_save_quest(struct nq_quest *quest) { return 0; }
...
void nq_interface_immo_list(P_char ch, char *args)
{
	...
	for (quest = nq_list; quest; quest = quest->next)
	{
		...
		if (quest->actor)
		{
			...
			snprintf(buf, 256, "  Holder: %s (in %d)\r\n", exists ? tch->player.short_descr : "&+RMISSING&n", exists ? world[tch->in_room].number : 0);
			send_to_char(buf, ch);
		}
		...
		for (instance = quest->instance; instance; instance = instance->next)
		{
			...
			for (actor = instance->actor; actor; actor = actor->next)
			{
				...
				snprintf(buf, 256, "    %s (in %d)\r\n", exists ? actor->ch->player.short_descr : "&+RMISSING&n", exists ? world[actor->ch->in_room].number : 0);
				send_to_char(buf, ch);
			}
		}
	}
