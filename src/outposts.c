...
int get_current_outpost_hitpoints(Building *building) { return 0; }

void outpost_update_resources(P_char ch, int wood, int stone) {}

int  outpost_rubble(P_obj obj, P_char ch, int cmd, char *arg) { return 0; }
...
void event_outpost_repair(P_char op, P_char vict, P_obj obj, void *data)
{
	...
	GET_HIT(building->get_mob()) =
		BOUNDED(GET_HIT(building->get_mob()), GET_HIT(building->get_mob()) + (building_types[BUILDING_OUTPOST - 1].hitpoints / 10), building_types[BUILDING_OUTPOST - 1].hitpoints);
...
void reset_one_outpost(Building *building)
{
	...
	db_query("UPDATE outposts SET owner_id = '0', level = '8', walls = '1', archers = '0', meurtriere = '0', hitpoints = '%d', portal_room = '0' WHERE id = '%d'",
...
void outpost_create_wall(int location, int direction, int type)
{
	...
	if (create_walls(location, direction, NULL, 64, WALL_OUTPOST, value, -1, "&+Wan outpost wall&n", "&+WAn outpost wall is here to the %s.&n", 0))
	{
		SET_BIT(world[location].dir_option[direction]->exit_info, EX_BREAKABLE);
		SET_BIT(VIRTUAL_EXIT((world[location].dir_option[direction])->to_room, rev_dir[direction])->exit_info, EX_BREAKABLE);
	}
}
...
int outpost_gateguard_proc(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (pl && cmd == cmd_from_dir(blocked_dir))
	{
		...
		if ((GET_ASSOC(pl) == GET_ASSOC(ch)) || (!GET_ASSOC(ch)))
			allowed = TRUE;
		else if ((alliance = GET_ASSOC(ch)->get_alliance()) && ((GET_ASSOC(pl) == alliance->get_forgers()) || (GET_ASSOC(pl) == alliance->get_joiners())))
			allowed = TRUE;
		if (!allowed && pl->group)
		{
			struct group_list *gl;
	...
	if (get_outpost_meurtriere(building) && (cmd == CMD_GOTHIT || cmd == CMD_GOTNUKED))
	{
	...
	if (pl && (cmd == CMD_GOTHIT && !number(0, 15)) || (cmd == CMD_HIT || cmd == CMD_KILL))
	{
		...
		for (P_desc i = descriptor_list; i; i = i->next)
			if (!i->connected && !is_silent(i->character, TRUE) && IS_SET(i->character->specials.act, PLR_GCC) && IS_MEMBER(GET_A_BITS(i->character)) && (GET_ASSOC(i->character) == GET_ASSOC(ch)) &&
...
{
	if ((world[to].sector_type == SECT_CASTLE_WALL && world[from].sector_type != SECT_CASTLE_WALL && world[from].sector_type != SECT_CASTLE) ||
	    (world[from].sector_type == SECT_CASTLE_WALL && (world[to].sector_type != SECT_CASTLE && world[to].sector_type != SECT_CASTLE_WALL)))
		return TRUE;
	else
		return FALSE;
}
