...
void event_illithid_feeding(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	act("$n releases $N&n, pulling $s tentacles out of $S ear.", TRUE, ch, 0, victim, TO_NOTVICT);
	act("$n releases you, pulling $s tentacles out of your ear.", TRUE, ch, 0, victim, TO_VICT);
...
void do_drain(P_char ch, char *arg, int cmd)
{
	...
	act("&+MYou grip your victim's head in your tentacles, sending one deep into $S brain.  The feeding begins!", FALSE, ch, 0, victim, TO_CHAR);
	act("&+M$n grips $N&n&+M's head in $s tentacles, sending one deep into $S brain.", TRUE, ch, 0, victim, TO_NOTVICT);
...
void event_githyanki_neckbite(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	act("$n&n removes $s teeth from $N&n's neck, leaving a bloodless corpse.", TRUE, ch, 0, victim, TO_NOTVICT);
	act("$n&n removes $s teeth from your neck -- you feel terrible.", TRUE, ch, 0, victim, TO_VICT);
...
void do_gith_neckbite(P_char ch, char *arg, int cmd)
{
	...
	send_to_char("&+RYou sink your teeth deeply into their neck, relishing in the feast that is just beginning..\r\n", ch);
	act("&+R$n&+R sinks $s teeth into $N&n&+R's neck, clearly relishing the experience.", TRUE, ch, 0, victim, TO_NOTVICT);
	act("&+RA wave of pain runs through your entire body as $n&+R sinks $s teeth into your neck.", FALSE, ch, 0, victim, TO_VICT);
}
...
void event_illithid_feeding2(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	act("&+G$n releases $N&+G, pulling $s tentacles out of $S ear.", TRUE, ch, 0, victim, TO_NOTVICT);
...
void do_absorbe(P_char ch, char *arg, int cmd)
{
	...
	act("&+GYou grip your victim's head in your tentacles, sending one deep into $S brain.  The feeding begins!", FALSE, ch, 0, victim, TO_CHAR);
	act("&+G$n grips $N&+G's head in $s tentacles, sending one deep into $S brain.", TRUE, ch, 0, victim, TO_NOTVICT);
	act("&+RA wave of pain grips your brain, as $N begins to slowly suck it out through your ear.", FALSE, ch, 0, victim, TO_VICT);
}
...
void spell_molecular_agitation(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	int                    dam_each[61] = {0,  0,  0,  0,  0,  12, 15, 18, 21, 24, 24, 24, 25, 25, 26, 26, 26, 27, 27, 27, 28, 28, 28, 29, 29, 29, 30, 30, 30, 31, 31,
	                                       31, 32, 32, 32, 33, 33, 33, 34, 34, 34, 35, 35, 35, 36, 36, 36, 37, 37, 37, 38, 38, 38, 38, 39, 39, 39, 39, 40, 40, 41};
	int                    dam;
	struct damage_messages messages = {"$N's entire body quivers in deliciously searing full-body pain.",
...
void spell_adrenaline_control(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	{
		send_to_char("Any more of an adrenaline rush, and yer heart would stop.\r\n", ch);
		return;
	}
	bzero(&af, sizeof(af));
...
void spell_awe(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (IS_NPC(victim) && !GET_MASTER(victim))
	{
		...
		if (total_levels + GET_LEVEL(victim) >= max_total_levels || GET_LEVEL(victim) > max_level)
		{
			if (!IS_FIGHTING(victim) && !IS_DESTROYING(victim))
			{
				act("$N&n seems really &+RPISSED OFF!&n", FALSE, ch, 0, victim, TO_CHAR);
				act("You see $n gazing at you, then charges $m!&n", FALSE, ch, 0, victim, TO_VICT);
				act("$N looks at $n, displays an &+rupset&n look, then charges $m!&n", FALSE, ch, 0, victim, TO_NOTVICT);
		...
		if (!IS_GREATER_RACE(victim) && !IS_ELITE(victim) && !IS_UNDEAD(victim) && !IS_TRUSTED(victim) &&
		    (OLD_RACE_EVIL(GET_RACE(victim), GET_ALIGNMENT(victim)) || OLD_RACE_GOOD(GET_RACE(victim), GET_ALIGNMENT(victim)) || GET_RACE(victim) == RACE_SNOW_OGRE ||
		     GET_RACE(victim) == RACE_RAKSHASA || GET_RACE(victim) == RACE_HUMANOID))
		{
			...
			if (save < number(0, 100) && !IS_TRUSTED(victim) && !IS_FIGHTING(victim) && !IS_DESTROYING(victim))
			{
				set_fighting(victim, ch);
				return;
			}
...
void spell_mind_travel(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	int r_room    = -1;
	int to_room[] = {96563, 96563, 11545, 15273, 96900, 36171, 96563, 4109, 4437, 4109, 96803, 17589, 17607, 96909, 3404, 23812, 23805, 12535, 12528, 25458, 25459, 25458, 12535, 12536, 12540};

	if (IS_FIGHTING(ch) || IS_DESTROYING(ch))
	{
		act("$n travels with the speed of &+Clight&n and reappears at the same place...", FALSE, ch, obj, 0, TO_CHAR);
		act("You travel with the speed of &+Clight&n and reappear at the same place", FALSE, ch, obj, 0, TO_NOTVICT);
		return;
	}
	...
	act("$n travel with the speed of &+Clight&n and reapear elsewhere...bye!", FALSE, ch, obj, 0, TO_CHAR);
	act("You travel with the speed of &+Clight&n and reapear elsewhere...", FALSE, ch, obj, 0, TO_CHAR);
...
{
	const int dam_each[61] = {0,  3,  4,  4,  5,  6,  6,  6,  7,  7,  7,  7,  7,  8,  8,  8,  9,  9,  9,  10, 10, 10, 11, 11, 11, 12, 12, 12, 13, 13, 13,
	                          14, 14, 14, 15, 15, 15, 16, 16, 16, 17, 17, 17, 18, 18, 18, 19, 19, 19, 20, 20, 21, 21, 21, 22, 22, 22, 23, 23, 23, 24};
	int       dam;
	...
	act("&+LYou chuckle as a stone strikes $N&+L.", TRUE, ch, 0, victim, TO_CHAR);
	act("&+L$n&+L creates a boulder, striking you solidly!", TRUE, ch, 0, victim, TO_VICT);
	act("&+L$n&+L squints, and a boulder appears, striking $N&+L solidly!", TRUE, ch, 0, victim, TO_NOTVICT);

	if (!StatSave(victim, APPLY_DEX, 0))
	{
		act("&+LYour mental boulder smashes $N&+L really hard.", TRUE, ch, 0, victim, TO_CHAR);
		act("&+LYou are almost smashed by the force of the boulder.!", TRUE, ch, 0, victim, TO_VICT);
		act("&+L$N&+L is almost smashed by the heavy projectile!", TRUE, ch, 0, victim, TO_NOTVICT);
		dam *= 2;
	}
...
void spell_cell_adjustment(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	{
		act("&+GYour willpower has neutralized the poison in your bloodstream.", FALSE, ch, 0, victim, TO_CHAR);
		poison_common_remove(victim);
	}

	if (curse && affected_by_spell(ch, SPELL_CURSE))
	{
		act("You tell the gods to fuck off and remove their curse!", FALSE, ch, 0, victim, TO_CHAR);
	...
	{
		act("&+rYour sheer willpower causes your body to shrug off the filthy withered sensation.", FALSE, ch, 0, victim, TO_CHAR);
		affect_from_char(victim, SPELL_WITHER);
	}

	if (disease && affected_by_spell(ch, SPELL_DISEASE))
	{
		act("&+rWith nothing but sheer willpower, you shrug off the putrid disease!", FALSE, ch, 0, victim, TO_CHAR);
	...
	{
		act("&+WYour vision returns, just as you willed it to.", FALSE, ch, 0, victim, TO_CHAR);
...
void spell_control_flames(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	const int dam_each[61] = {0,  0,  0,  0,  0,  0,  0,  0,  16, 20, 24, 28, 32, 35, 38, 40, 42, 44, 45, 45, 45, 46, 46, 46, 47, 47, 47, 48, 48, 48, 49,
	                          49, 49, 50, 50, 50, 51, 51, 51, 52, 52, 52, 53, 53, 53, 54, 54, 54, 55, 55, 55, 56, 56, 56, 57, 57, 58, 58, 58, 59, 59};
...
void spell_ego_blast(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	*/
	...
	act("&+M$n squints and $N&+M screams out in great pain, holding $S head!", TRUE, ch, 0, victim, TO_NOTVICT);
...
void spell_create_sound(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	{
		act("&+WYour sonic boom of mental power dispels the silence!", 0, ch, 0, 0, TO_CHAR);
...
void spell_psychic_crush(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	char                   buf[256];
	struct affected_type   af;
	struct damage_messages messages = {"$N's &+meyes briefly cloud over as $S entire psyche is wracked with wonderfully intense pain.",
	                                   ...
	                                   "&+RBlood &+mseeps from&n $N's &+mears as $S mind is torn to &+wpieces.",
	                                   0};
	if (!ch)
	{
	...
	if (!affected_by_spell(victim, SPELL_PSYCHIC_CRUSH))
	{
		...
		act("$n &+msuddenly clutches their head, as if trying to shrug off some unknown malady.&n", FALSE, victim, 0, 0, TO_ROOM);
	...
	if (GET_SPEC(ch, CLASS_PSIONICIST, SPEC_ENSLAVER) &&
	    !(IS_ELITE(victim) || IS_TRUSTED(victim) || IS_ZOMBIE(victim) || GET_RACE(victim) == RACE_GOLEM || GET_RACE(victim) == RACE_DRAGON || GET_RACE(victim) == RACE_PLANT))
	{
		...
		else if (spec_affect < 15)
		{
			...
			act("&+yYou fall to your knees as uncontrollable pain engulfs your entire brain!&n", FALSE, ch, NULL, victim, TO_VICT);
			act("$N &+yfalls to their knees, grasping $s head and screaming in pain!&n", FALSE, ch, NULL, victim, TO_NOTVICT);
			act("$N &+yfalls to their knees, grasping $s head and screaming in pain!!&n", TRUE, ch, NULL, victim, TO_CHAR);
...
void spell_death_field(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (IS_PC(ch) && (tch = stack_area(ch, SPELL_DEATH_FIELD, (int)get_property("spell.area.stackTimer.deathField", 5))))
	{
		act("&+LYour deadly wave interferes with $N's field!", FALSE, ch, 0, tch, TO_CHAR);
	...
	cast_as_damage_area(ch, spell_single_death_field, level, victim, get_property("spell.area.minChance.deathField", 60), get_property("spell.area.chanceStep.deathField", 30));
...
{
	int                    dam;
	struct damage_messages messages = {"You feel a warm sense of satisfaction as &+Rexplosions&N engulf $N.",
	...
	if (/*get_linked_char(victim, LNK_PET) ||*/ GET_RACE(victim) == RACE_GOLEM || GET_RACE(victim) == RACE_PLANT)
		dam = (dam * 2);
		...
		switch (number(1, 5))
		{
			...
			case 4:
				if (!affected_by_spell(victim, SPELL_SPINAL_CORRUPTION))
				{
					...
					affect_to_char(victim, &af);
					act("&+LThe power of your psychic assault renders $N unable to react as quickly!", TRUE, ch, 0, victim, TO_CHAR);
...
void spell_mental_anguish(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	else
	{
		...
		act("$N's eyes roll back as $S brain is filled with raw pain.", TRUE, ch, 0, victim, TO_CHAR);
		act("&+RYour brain is filled with pain, making it hard to concentrate!&n", TRUE, ch, 0, victim, TO_VICT);
...
void spell_memory_block(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("You seem to have forgotten why you are here!&n", TRUE, ch, 0, victim, TO_VICT);
...
void spell_psionic_cloud(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("&+LYou summon a dark haze of psionic energy with the power of your mind!", FALSE, ch, 0, 0, TO_CHAR);
	act("&+L$n &+Lsquints and a dark haze of psionic energy appears!", FALSE, ch, 0, 0, TO_ROOM);
...
void spell_domination(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (GET_MASTER(victim) || GET_MASTER(ch) || level < GET_LEVEL(victim) || StatSave(victim, APPLY_POW, POW_DIFF(ch, victim)))
		return;
	/*
	   if (victim->leader)
	   stop_follower(victim);
	...
	{ /*
	   ...
	   */
		if (!affected_by_spell(victim, SPELL_ECTOPLASMIC_FORM))
		{
			...
			act("&+L$n's form shimmers and then fades from sight..", TRUE, victim, 0, 0, TO_ROOM);
...
void spell_ego_whip(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("&+M$n squints and $N&+M screams out in pain, holding $S head!", TRUE, ch, 0, victim, TO_NOTVICT);
...
void spell_enhanced_strength(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("$n looks physically stronger.", TRUE, victim, 0, 0, TO_ROOM);
...
{
	struct damage_messages messages = {"$N &+Bscreams out in &+Rpain!&N", "&+BA horrible &+Rpain&N wracks your brain!", "$N &+Bscreams out in &+Rpain!&N"};
...
void spell_lend_health(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (hp <= 0)
	{
		act("Nice thought, but $N doesn't need healing.", 0, ch, 0, victim, TO_CHAR);
		return;
	}
	if (GET_HIT(ch) - hp < -9)
	{
...
void spell_pyrokinesis(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	struct damage_messages messages = {
		...
		"&+mYou scream in &+Wa&+wg&+Wo&+wn&+Wy &+mas &+rs&+Rea&+rri&+Rng &+rh&+Re&+Ra&+rt &+wi&+rg&+wn&+wi&+rt&+we&+rs and &+rb&+Rur&+Ws&+wt&+Rs &+Ri&+Wn&+wt&+Ro f&+Wl&+wa&+Wm&+Re&+Rs &+mwithin your "
		"body.",
		...
		"&+RRa&+rgi&+Rng &+rf&+Rla&+rme&+Rs &+Wb&+wu&+Wr&+ws&+Wt &+mfrom within and &+We&+wn&+Wg&+wu&+Wl&+wf &N$N's &+mwhole body, leaving nothing but &+Lscorched &+mremains.",
		0};
	...
	if (!affected_by_spell(victim, SPELL_PYROKINESIS))
	{
		...
		act("&+m$n &+msuddenly looks in pain as $e moves.&n", FALSE, victim, 0, 0, TO_ROOM);
...
void spell_celerity(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("&+WYou concentrate and attempt to transform part of your lifeforce....&n", FALSE, ch, 0, victim, TO_CHAR);
	act("$N &+Wconcentrates and pales visibly.&n", FALSE, ch, 0, victim, TO_NOTVICT);

	if (spell_damage(ch, ch, movepoints * 4, SPLDAM_PSI, SPLDAM_NODEFLECT | SPLDAM_NOSHRUG, 0) != DAM_NONEDEAD)
		return;

	GET_VITALITY(ch) = MIN(GET_VITALITY(ch) + movepoints, GET_MAX_VITALITY(ch));

	act("&+yFresh energy pours through your body. &+WYou are invigorated!&n", FALSE, ch, 0, 0, TO_CHAR);
	act("$N &+Wappears invigorated.&n", FALSE, ch, 0, 0, TO_ROOM);
...
void spell_depart(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("You will yourself to be elsewhere...&n", FALSE, victim, 0, 0, TO_CHAR);
	act("A &+Lblack line&n forms from nowhere and $n falls through it!&n", FALSE, victim, 0, 0, TO_ROOM);
	...
	act("and appear somewhere else!&n", FALSE, victim, 0, 0, TO_CHAR);
...
{
	struct damage_messages messages = {"You begin your mental assault upon $N.&n",
	...
	if (spell_damage(ch, victim, dam, SPLDAM_PSI, RAWDAM_NOKILL | SPLDAM_NODEFLECT | SPLDAM_NOSHRUG, 0) != DAM_NONEDEAD)
		return;
...
void event_psionic_wave_blast(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	if (af && ((GET_POS(ch) < POS_STANDING) || IS_STUNNED(ch)))
	{
	...
	if (!number(0, 2) && resists_spell(ch, victim))
	{
		...
		add_event(event_psionic_wave_blast, PULSE_VIOLENCE, ch, victim, NULL, 0, &level, sizeof(level));
		return;
	}
	...
	if (spell_damage(ch, victim, dam, SPLDAM_PSI, SPLDAM_NODEFLECT | SPLDAM_NOSHRUG, &messages) == DAM_NONEDEAD)
	{
		if (!IS_SET((ch)->specials.affected_by2, AFF2_CASTING))
			SET_BIT((ch)->specials.affected_by2, AFF2_CASTING);
...
void spell_innate_blast(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (!number(0, 2) && !StatSave(victim, APPLY_POW, MAX(-3, (GET_LEVEL(ch) - GET_LEVEL(victim)) / 3)) && !(IS_NPC(victim) && IS_SET(victim->specials.act, ACT_IMMUNE_TO_PARA)))
	{
...
void spell_radial_navigation(int level, P_char ch, char *arg, int type, P_char victim, P_obj tar_obj)
{
	...
	if (!IS_TRUSTED(ch) && ((to_room == NOWHERE) || (to_room == ch->in_room) || !IS_MAP_ROOM(ch->in_room) || !IS_MAP_ROOM(to_room) || IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) ||
	                        IS_ROOM(to_room, ROOM_NO_TELEPORT) || (GET_MASTER(ch) && IS_PC(victim))))
	{
		act("&+L$n's&+L outline begins to &n&+rvibrate&+L, then &+Bblur&+L.... then stabilize.&N", TRUE, ch, 0, 0, TO_ROOM);
		act("&+wYour will is not strong enough to carry you away.&n", TRUE, ch, 0, 0, TO_CHAR);
	}
	else
	{
		act("&+L$n's&+L outline begins to &n&+rvibrate&+L, then &+Bblur&+L, then.....    disappear.&N", TRUE, ch, 0, 0, TO_ROOM);
		char_from_room(ch);
		char_to_room(ch, to_room, -1);
		send_to_char("&+LYou feel exhausted from the jaunt.&N\r\n", ch);
		act("&+LFrom out of nowhere $n's&+L blurred form streaks into the room!&N", TRUE, ch, 0, 0, TO_ROOM);
	}

	/* subtract mana regardless of success..  snif */
	GET_MANA(ch) -= (distance * get_property("spells.radialnavigation.manaburnfactor", 12));
...
void spell_ethereal_rift(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	do
	{
		to_room = number(zone_table[world[victim->in_room].zone].real_bottom, zone_table[world[victim->in_room].zone].real_top);
		found++;
	} while ((IS_ROOM(to_room, ROOM_PRIVATE) || PRIVATE_ZONE(to_room) || IS_ROOM(to_room, ROOM_NO_TELEPORT) || IS_HOMETOWN(to_room) || world[to_room].sector_type == SECT_OCEAN) && found < 1000);

	if ((IS_ROOM(victim->in_room, ROOM_PRIVATE) || PRIVATE_ZONE(victim->in_room) || IS_ROOM(victim->in_room, ROOM_NO_TELEPORT) || IS_HOMETOWN(victim->in_room) ||
	     world[victim->in_room].sector_type == SECT_OCEAN))
	{
		to_room = ch->in_room;
	}

	if ((IS_ROOM(ch->in_room, ROOM_PRIVATE) || PRIVATE_ZONE(ch->in_room) || IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) || IS_HOMETOWN(ch->in_room) || (world[ch->in_room].sector_type == SECT_OCEAN)))
	{
		to_room = ch->in_room;
	}
	...
	send_to_char("Your mind catapults you through a tear in the ethereal fabric.\r\n", ch);
	...
	for (tmp = world[ch->in_room].people; tmp; tmp = tmp_next)
	{
		...
		if (IS_AFFECTED(tmp, AFF_BLIND) || (tmp == ch))
			continue;
		act("&+LA dark tear in reality opens, and $n shimmers and fades away!\r\n", FALSE, ch, 0, tmp, TO_VICT);
	}
	char_from_room(ch);
	ch->specials.z_cord = victim->specials.z_cord;
	char_to_room(ch, to_room, -1);
	send_to_char("Your body aches as your molecules reassemble themselves. \r\n", ch);
	...
	zone_powerspellmessage(to_room, "&+LYou sense a disturbance in the ethereal fabric.\r\n");
...
void spell_ether_warp(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (IS_PC(victim) && IS_ILLITHID(ch) && !IS_TRUSTED(ch) && !IS_ILLITHID(victim) && GET_LEVEL(victim) < 51)
	{
		...
		wizlog(57, "%s attmpted to wormhole %s, and made the &+MElder Brain&n displeased.", GET_NAME(ch), GET_NAME(victim));
	...
	to_room = victim->in_room;
	temp    = world[ch->in_room].sector_type;
	if (!IS_TRUSTED(ch) /*  && !IS_ILLITHID(ch) */ && ((to_room == NOWHERE) || (to_room == ch->in_room) || IS_TRUSTED(victim) || IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) || IS_HOMETOWN(ch->in_room) ||
	                                                   IS_HOMETOWN(to_room) || IS_ROOM(to_room, ROOM_NO_MAGIC) || IS_ROOM(to_room, ROOM_NO_TELEPORT) || world[to_room].sector_type == SECT_OCEAN ||
	                                                   IS_AFFECTED3(victim, AFF3_NON_DETECTION) || (IS_NPC(victim) && (!GET_SPEC(ch, CLASS_PSIONICIST, SPEC_PSYCHEPORTER) && !IS_ILLITHID(ch))) ||
	                                                   (IS_PC(victim) && IS_SET(victim->specials.act2, PLR2_NOLOCATE) && !is_introd(victim, ch)) || racewar(ch, victim) ||
	...
	if (!IS_TRUSTED(ch) /* && IS_ILLITHID(ch) */ && ((to_room == NOWHERE) || (to_room == ch->in_room) || IS_TRUSTED(victim) || IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) || IS_HOMETOWN(ch->in_room) ||
	                                                 IS_HOMETOWN(to_room) || IS_ROOM(to_room, ROOM_NO_MAGIC) || IS_ROOM(to_room, ROOM_NO_TELEPORT) || world[to_room].sector_type == SECT_OCEAN ||
	                                                 IS_AFFECTED3(victim, AFF3_NON_DETECTION) || (IS_NPC(victim) && (!GET_SPEC(ch, CLASS_PSIONICIST, SPEC_PSYCHEPORTER) && !IS_ILLITHID(ch))) ||
	                                                 (IS_PC(victim) && IS_SET(victim->specials.act2, PLR2_NOLOCATE) && !(temp == world[to_room].sector_type))))
	{
		to_room = ch->in_room;
	}

	distance = (int)(level * get_property("spell.dim.perlevel.modifier", 1.35));
	if (!IS_TRUSTED(ch) && IS_NPC(victim) && (GET_SPEC(ch, CLASS_PSIONICIST, SPEC_PSYCHEPORTER) || IS_ILLITHID(ch)) && how_close(ch->in_room, victim->in_room, distance) < 0 &&
	...
	{
		char logbuf[500];
		snprintf(logbuf, 500, "Ether Warp from %s[%d] to %s[%d]", GET_NAME(ch), world[ch->in_room].number, GET_NAME(victim), world[to_room].number);
...
void spell_wormhole(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	struct portal_create_messages msg = {/*ch   */ "&+LThe rift swirls for a moment and disperses.\r\n",
	...
	*/
	if (ch) // Just making sure... Nov08 -Lucrot
	{
		...
		if (IS_NPC(victim))
		{
			if (!has_innate(ch, INNATE_IMPROVED_WORMHOLE) || IS_SET(victim->specials.act, ACT_NO_SUMMON))
				success = false;
			else
			{
				distance = (int)(level * get_property("spell.dim.perlevel.modifier", 1.35));

				if (how_close(ch->in_room, to_room, distance) < 0 && how_close(to_room, ch->in_room, distance) < 0)
					success = false;
			}
		}
		...
		if (victim && IS_NPC(victim) && victim != ch && !IS_TRUSTED(ch) && has_innate(ch, INNATE_IMPROVED_WORMHOLE) && IS_SET(victim->specials.act, ACT_NO_SUMMON))
		{
		...
		if (!can_do_general_portal(level, ch, victim, &set, &msg) || (!IS_TRUSTED(ch) && (!success || GET_MASTER(ch))))
		{
			act(msg.fail_to_caster, FALSE, ch, 0, 0, TO_CHAR);
			act(msg.fail_to_caster_room, FALSE, ch, 0, 0, TO_ROOM);
			return;
...
void spell_excogitate(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (ch)
	{
		...
		if (diff < -20)
		{
			act("$N's mental power is &+yfar greater than yours.&n", TRUE, ch, 0, victim, TO_CHAR);
