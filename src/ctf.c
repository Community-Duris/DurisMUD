...
int ctf_flag_proc(P_obj flag, P_char ch, int cmd, char *argument)
{
	...
	if (GET_RACEWAR(ch) != RACEWAR_GOOD && GET_RACEWAR(ch) != RACEWAR_EVIL)
		return FALSE;
...
int ctf_use_boon(BoonData *bdata)
{
	...
	// double check
	if (ctfdata[i].id && ctfdata[i].type == CTF_BOON && !ctfdata[i].room)
	{
...
int ctf_get_random_room(int id)
{
	...
	while (!room)
	{
		...
		for (obj = object_list; obj; obj = obj->next)
		{
			...
			{
				if (ctfdata[i].room > 0 && ctfdata[i].room == room)
				{
					room = 0;
					continue;
				}
