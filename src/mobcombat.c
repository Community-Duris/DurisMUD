...
void ZombieCombat(P_char ch, P_char victim)
{
	...
	else if (random <= 10 && GET_LEVEL(ch) >= 31)
	{
		do_action(ch, 0, CMD_LICK);
		act("$n savagely bites into your flesh!", TRUE, ch, 0, victim, TO_VICT);
	...
	else if (!number(0, 9) && (GET_LEVEL(ch) >= 56) && !affected_by_spell(victim, SPELL_PLAGUE))
	{
...
void SpectreCombat(P_char ch, P_char victim)
{
	...
	if (random == 1)
	{
		act("You hear a low groaning sound from $n.", TRUE, ch, 0, 0, TO_ROOM);
		do_action(ch, 0, CMD_GROAN);
	}
	else if (random <= 3)
	{
...
int AngelCombat(P_char ch)
{
	...
	if (GET_RACE(ch) == RACE_BRALANI && victim && number(0, 1) && GOOD_FOR_GAZING(ch, victim))
	{
		innate_gaze(ch, victim);
	}
...
int BeholderCombat(P_char ch)
{
	...
	{
		if ((tch != ch) && ((IS_PC(tch) && !IS_TRUSTED(tch)) || (GET_OPPONENT(tch) == ch)))
		{
			numbPCs++;
			if (!firstPC)
			{
	...
	if (gotlucky && !hitpc && firstPC && char_in_list(firstPC))
	{
...
bool Mob_Furious(P_char ch, P_char victim, int chance)
{
	...
	if (GET_C_DEX(ch) < 100 && !number(0, 3))
		ch->base_stats.Dex = 100;

	if (GET_C_AGI(ch) < 100 && !number(0, 3))
		ch->base_stats.Agi = 100;

	if (GET_C_POW(ch) < 100 && !number(0, 3))
		ch->base_stats.Pow = 100;

	if (GET_C_CON(ch) < 100 && !number(0, 3))
