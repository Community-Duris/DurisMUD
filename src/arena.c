...
const int max_team_size[] = {MAX_GOODIE_TEAM, MAX_EVIL_TEAM, MAX_UNDEAD_TEAM};

const char *game_type[] = {"Capture the Flag", "Jail Break", "King of the Hill", "Team DeathMatch", "IT", "Deathmatch", "\n"};

const char *arena_stage[] = {"&+GAccepting players&N", "&+rPre Match prep&N", "&=LRMatch in progress&N", "&=LRMatch in progress&N", "&+LPost match aftermath&N", "&+LPost match aftermath&N"};
const char *death_type[]  = {"Winner Takes ALL", "Even Trade", "Entrance Fee, Winner takes Pot", "Free Play", "\n"};

const char *map_name[] = {"Opposing Castles", "\n"};

extern const int arena_hometown_location[];
const int        arena_hometown_location[] = {6075,  // Goodie
                                              29700, // EVIL
                                              -1};
		...
		if (d->connected == CON_PLAYING)
		{
			...
			if (race == -1)
			{
				for (i = 0; i < MAX_RACES; i++)
				{
					...
					{
						if (real_room(arena_hometown_location[i]) == d->character->in_room)
						{
							send_to_char(msg, d->character);
						}
					}
...
char *arena_death_msg(P_obj p_weapon) { return "killed"; }
...
int arena_team_count(int team)
{
	...
	if ((team < MAX_RACES) && (team > -1))
	{
		...
		{
			if (!IS_SET(arena.team[team].player[i].flags, PLAYER_DEAD) && arena.team[team].player[i].ch != NULL)
				count++;
		}
	}
	return count;
...
void players_to_map()
{
	...
	for (i = 0; i < MAX_RACES; i++)
	{
		...
		for (j = 0; j < 2; j++)
		{
			for (ch = world[real_room(arena_start_room[i][j])].people; ch; ch = next_ch)
			{
				if (ch == NULL)
					break;
	...
	for (d = descriptor_list; d; d = d->next)
	{
		if (d->character != NULL)
		{
			if (CHAR_IN_ARENA(d->character))
			{
				...
				char_to_room(d->character, real_room(arena_hometown_location[arena_team(d->character)]), -1);
			}
		}
	}
}
void show_stats_to_char(P_char ch)
{
	...
	else
	{
		...
		snprintf(tbuf, MAX_STRING_LENGTH, "&+LGame Type: &+W%-20s &+LDeath Mode: &+W%s\r\n&N", game_type[arena.type], death_type[arena.deathmode]);
		send_to_char(tbuf, ch);
		countg = 0;
		counte = 0;
		for (tch = world[real_room0(arena_start_room[GOODIE][0])].people; tch; tch = tch->next_in_room)
		{
		...
		for (tch = world[real_room0(arena_start_room[EVIL][0])].people; tch; tch = tch->next_in_room)
		{
			if (!IS_TRUSTED(tch) && IS_PC(tch))
			{
				counte++;
		...
		snprintf(tbuf, MAX_STRING_LENGTH, "&+LPeople in arena - &+YGoodies: &+W%-2d &+rEvils: &+W%-2d&N\r\n", countg, counte);
		send_to_char(tbuf, ch);
...
int arenaobj_proc(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_ENTER)
	{
		...
		{
			send_to_char("&+YMatch currently in progress, wait for the next one.&N\r\n", ch);
			return TRUE;
		}
		...
		for (i = 0; i < 2; i++)
		{
			for (temp = world[real_room(arena_start_room[race][i])].people; temp; temp = temp->next_in_room)
			{
				if (!temp)
					continue;
		...
		{
			send_to_char("&+LI'm sorry but the team is currently full, please wait for the next match.&N\r\n", ch);
			return TRUE;
		}
		...
		{
			char_from_room(ch);
			char_to_room(ch, real_room(arena_hometown_location[IS_RACEWAR_EVIL(ch) ? EVIL : GOODIE]), -1);
			act("$n steps out of $p.", TRUE, ch, obj, 0, TO_ROOM);
			return TRUE;
		}
	}
...
void arena_activity()
{
	...
	if (IS_SET(arena.flags, FLAG_ENABLED))
	{
		if (IS_SET(arena.flags, FLAG_SHUTTING_DOWN))
		{
			...
			players_from_map();
			send_to_arena("&+WARENA SHUTTING DOWN, ALL PLAYERS BEING EVICTED.&N\r\n", -1);
		...
		if (arena.stage == STAGE_OPEN)
		{
			...
			{
				case 0:
					send_to_arena("&+WArena Entrance is now closed, let's get ready to RUMBLE!&N\r\n", -1);
					break;
				case 10:
		...
		if (arena.stage == STAGE_ACCEPT)
		{
			...
			if (arena.timer[0] < 1)
			{
				...
				if (j < 2 && arena.type != TYPE_DEATHMATCH && arena.type != TYPE_KING_OF_THE_HILL)
				{
					send_to_arena("&+LSince one side has chickened out, next match will begin in 15 minutes.&N\r\n", -1);
					arena.stage--;
					arena.timer[0] = (15 * 60);
					return;
				}
		...
		{
			players_from_map();
			send_to_arena("&+WThe Match has ended the ending scores are as follows:&N\r\n", -1);
			show_scores(-1);
			arena.stage++;
