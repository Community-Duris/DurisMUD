...
P_obj get_random_enemy_nexus(P_char ch) { return NULL; }

P_obj get_nexus_stone(int stone_id) { return NULL; }
int   check_nexus_bonus(P_char ch, int amount, int type) { return 0; }
#else
...
char *ns_messages[] = {"&+gA strange field of energy prevents you from touching $p&+G.&n",
...
bool nexus_stone_touch(P_obj stone, P_char ch)
{
	...
	for (i = world[ch->in_room].people; i; i = next)
	{
		...
		if (IS_NPC(i))
		{
			if (GET_VNUM(i) == MOB_NEUTRAL_GUARDIAN || GET_VNUM(i) == MOB_GOOD_GUARDIAN || GET_VNUM(i) == MOB_EVIL_GUARDIAN)
			{
	...
	if ((IS_RACEWAR_GOOD(ch) && STONE_ALIGN(stone) >= STONE_ALIGN_GOOD) || (IS_RACEWAR_EVIL(ch) && STONE_ALIGN(stone) <= STONE_ALIGN_EVIL))
	{
	...
	if ((IS_RACEWAR_GOOD(ch) && (STONE_ALIGN(stone) + 1) >= STONE_ALIGN_GOOD) || (IS_RACEWAR_EVIL(ch) && (STONE_ALIGN(stone) - 1) <= STONE_ALIGN_EVIL))
	{
		will_turn = true;
	}
...
void nexus_stone_epics(P_char ch, P_obj stone)
{
	...
	gain_epic(ch, EPIC_NEXUS_STONE, STONE_ID(stone), number(amount_min, amount));
	...
	if (ch->group)
	{
		for (struct group_list *gl = ch->group; gl; gl = gl->next)
		{
			...
			if (gl->ch->in_room == ch->in_room)
			{
				if ((afp = get_epic_task(gl->ch)) && ((abs(afp->modifier) - SPILL_BLOOD) == STONE_ID(stone)))
				{
					send_to_char("The &+rGods of Duris&n are very pleased with your success.\n", gl->ch);
					send_to_char("You can now progress further in your quest for epic power!\n", gl->ch);
					amount *= 2;
				...
				gain_epic(gl->ch, EPIC_NEXUS_STONE, STONE_ID(stone), number(amount_min, amount));
			}
		}
	}
}
...
void nexus_guardian_nuke(P_char ch, P_char victim, int dam)
{
	...
	struct damage_messages messages = {"Your beam lances into $N!",
	                                   ...
	                                   "&+WThe energy beam utterly annihilates $N!",
	                                   0};
...
void nexus_guardian_energy_burst(P_char ch, bool show_message = true)
{
	...
	if (show_message)
	{
		act("&+BSuddenly, $n &+Wexplodes into a blazing blue&+B eldritch flame. As he continues\n"
		    ...
		    "&+BSuddenly, a crackling wave of kinetic &+Wenergy&+B lashes out..headed directly at YOU!",
		    FALSE,
		    ch,
		    0,
		    0,
...
int nexus_guardian_pwn_mace(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_GOTHIT && !number(0, 5))
	{
		...
		act("&+wAs your mighty blow impacts on $n, time itself &+Wseems to stop.\n"
		    ...
		    "&+wblow is being &+Wsiphoned into $n!",
		    FALSE,
		    ch,
		    0,
		    victim,
		...
		act("&+wYou feel &+Mstrange&+w for a moment. Suddenly, reality seems to resume,\n"
		    "and the &+raura&+w around $N's weapon is absorbed by $n.",
		    FALSE,
		    ch,
		    0,
		    victim,
...
int nexus_sage(P_char ch, P_char pl, int cmd, char *arg)
{
	if (pl)
	{
		if ((IS_GOOD(ch) && !GOOD_RACE(pl)) || (IS_EVIL(ch) && !EVIL_RACE(pl)))
		{
			return FALSE;
		}
