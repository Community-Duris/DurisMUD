...
#define IS_IMMATERIAL(mob)                                                                                                                                                                             \
	((GET_RACE(mob) == RACE_WRAITH) || (GET_RACE(mob) == RACE_GHOST) || (GET_RACE(mob) == RACE_SHADE) || (GET_RACE(mob) == RACE_A_ELEMENTAL) || (GET_RACE(mob) == RACE_F_ELEMENTAL) ||                 \
	 (GET_RACE(mob) == RACE_PHANTOM) || (GET_RACE(mob) == RACE_WIGHT) || (GET_RACE(mob) == RACE_SHADOW) || (GET_RACE(mob) == RACE_DEVA) || (GET_RACE(mob) == RACE_SPECTRE) ||                          \
	 (GET_RACE(mob) == RACE_V_ELEMENTAL) || (GET_RACE(mob) == RACE_BRALANI) || IS_AFFECTED4(mob, AFF4_PHANTASMAL_FORM))

#define LEGLESS(mob)                                                                                                                                                                                   \
	(IS_IMMATERIAL(mob) || IS_BEHOLDER(mob) || (GET_RACE(mob) == RACE_SLIME) || (GET_RACE(mob) == RACE_CONSTRUCT) || (GET_RACE(mob) == RACE_SNAKE) || (GET_RACE(mob) == RACE_INSECT) ||                \
	 (GET_RACE(mob) == RACE_PARASITE) || (GET_RACE(mob) == RACE_AQUATIC_ANIMAL) || (GET_RACE(mob) == RACE_PLANT) || (GET_RACE(mob) == RACE_EFREET) || (GET_RACE(mob) == RACE_PWORM))
...
#define OUTDOOR_SNEAK(ch) (has_innate(ch, INNATE_OUTDOOR_SNEAK) && (world[(ch)->in_room].sector_type >= 2) && (world[(ch)->in_room].sector_type <= 4))
#define UD_SNEAK(ch)      (has_innate(ch, INNATE_UD_SNEAK) && IS_UNDERWORLD(ch->in_room))

#define SWAMP_SNEAK_TERRAIN(ch) ((world[(ch)->in_room].sector_type == SECT_SWAMP) || (world[(ch)->in_room].sector_type == SECT_UNDRWLD_SLIME))
...
#define BAD_APPLYS(apply)                                                                                                                                                                              \
	(apply == APPLY_GOLD || apply == APPLY_CHAR_WEIGHT || apply == APPLY_CHAR_HEIGHT || apply == APPLY_EXP || apply == APPLY_CLASS || apply == APPLY_LEVEL || apply == APPLY_SEX ||                    \
	 apply == APPLY_FIRE_PROT)

#define IS_FULLY_RESTED(ch, skill_type, now) (((now) - (ch)->specials.skill_usage[(skill_type)].time_of_first_use) >= SECS_PER_MUD_DAY)

#define SKILL_HOURS_TO_REST(ch, skill_type, now) (24 - (((now) - (ch)->specials.skill_usage[(skill_type)].time_of_first_use) * 24 / SECS_PER_MUD_DAY))
...
#define HAS_VEGETATION(sect)                                                                                                                                                                           \
	(sect == SECT_FIELD || sect == SECT_FOREST || sect == SECT_HILLS || sect == SECT_MOUNTAIN || sect == SECT_UNDRWLD_WILD || sect == SECT_DESERT || sect == SECT_ARCTIC || sect == SECT_SWAMP ||      \
	 sect == SECT_UNDRWLD_MOUNTAIN || sect == SECT_UNDRWLD_SLIME || sect == SECT_UNDRWLD_MUSHROOM || sect == SECT_SNOWY_FOREST || sect == SECT_UNDRWLD_WILD || sect == SECT_UNDRWLD_LOWCEIL ||         \
	 sect == SECT_CITY || sect == SECT_UNDRWLD_CITY)
#define IS_SWAMP_ROOM(r) (world[r].sector_type == SECT_SWAMP)
#define IS_CASTLE(r)     (world[r].sector_type == SECT_CASTLE_WALL || world[r].sector_type == SECT_CASTLE_GATE || world[r].sector_type == SECT_CASTLE)

#define IS_UNDERWATER(c)                                                                                                                                                                               \
	((world[c->in_room].sector_type == SECT_UNDERWATER) || (world[c->in_room].sector_type == SECT_UNDERWATER_GR) || (world[c->in_room].sector_type == SECT_WATER_PLANE) ||                             \
	 (IS_WATER_ROOM(c->in_room) && c->specials.z_cord < 0) || (IS_ROOM(ch->in_room, ROOM_UNDERWATER)))

#define IS_WATER_ROOM(r)                                                                                                                                                                               \
	(world[r].sector_type == SECT_UNDRWLD_WATER || world[r].sector_type == SECT_UNDRWLD_NOSWIM || world[r].sector_type == SECT_OCEAN || world[r].sector_type == SECT_UNDERWATER ||                     \
	 world[r].sector_type == SECT_UNDERWATER_GR || world[r].sector_type == SECT_WATER_NOSWIM || world[r].sector_type == SECT_WATER_SWIM || world[r].sector_type == SECT_WATER_PLANE)

#define IS_UNDERWORLD(r)                                                                                                                                                                               \
	(world[r].sector_type == SECT_UNDRWLD_CITY || world[r].sector_type == SECT_UNDRWLD_INSIDE || world[r].sector_type == SECT_UNDRWLD_WILD || world[r].sector_type == SECT_UNDRWLD_WATER ||            \
	 world[r].sector_type == SECT_UNDRWLD_NOSWIM || world[r].sector_type == SECT_UNDRWLD_NOGROUND || world[r].sector_type == SECT_UNDRWLD_MOUNTAIN || world[r].sector_type == SECT_UNDRWLD_SLIME ||    \
	 world[r].sector_type == SECT_UNDRWLD_LOWCEIL || world[r].sector_type == SECT_UNDRWLD_LIQMITH || world[r].sector_type == SECT_UNDRWLD_MUSHROOM)

#define NORMAL_PLANE(r)                                                                                                                                                                                \
	(world[r].sector_type != SECT_FIREPLANE && world[r].sector_type != SECT_WATER_PLANE && world[r].sector_type != SECT_EARTH_PLANE && world[r].sector_type != SECT_AIR_PLANE &&                       \
	 world[r].sector_type != SECT_ETHEREAL && world[r].sector_type != SECT_ASTRAL && world[r].sector_type != SECT_NEG_PLANE)

#define IS_LIGHT(r) ((world[r].light > 0 && !IS_ROOM(r, ROOM_MAGIC_DARK)) || IS_SUNLIT(r) || IS_TWILIGHT_ROOM(r) || IS_ROOM(r, ROOM_MAGIC_LIGHT))

#define CAN_DAYPEOPLE_SEE(r) (world[r].light > 0 || IS_SUNLIT(r) || IS_TWILIGHT_ROOM(r) || IS_ROOM(r, ROOM_MAGIC_LIGHT))
...
#define IS_SUNLIT(r) (IS_DAY && !IS_TWILIGHT && IS_OUTDOORS(r) && !IS_ROOM(r, ROOM_DARK | ROOM_MAGIC_DARK))
...
#define GET_REQ(i) (i < 1 ? "Awful" : (i < 3 ? "Bad" : (i < 5 ? "Poor" : (i < 7 ? "Average" : (i < 10 ? "Fair" : (i < 15 ? "Good" : (i < 24 ? "Very good" : "Superb")))))))
...
#define GET_MONEY(ch) ((ch)->points.cash[0] + 10 * (ch)->points.cash[1] + 100 * (ch)->points.cash[2] + 1000 * (ch)->points.cash[3])

#define GET_BALANCE(ch) ((ch)->only.pc->spare1 + 10 * (ch)->only.pc->spare2 + 100 * (ch)->only.pc->spare3 + 1000 * (ch)->only.pc->spare4)
...
#define GET_HEIGHT(ch) ((ush_int)((IS_AFFECTED3(ch, AFF3_ENLARGE) ? ((ch)->player.height * 1.5) : IS_AFFECTED3(ch, AFF3_REDUCE) ? ((ch)->player.height / 1.5) : (ch)->player.height)))

#define GET_WEIGHT(ch) (IS_AFFECTED3(ch, AFF3_ENLARGE) ? ((ch)->player.weight * 2) : IS_AFFECTED3(ch, AFF3_REDUCE) ? ((ch)->player.weight / 2) : (ch)->player.weight)
...
#define MIN_POS(ch, v) ((GET_POS(ch) >= (ubyte)((v) & 3)) && (GET_STAT(ch) >= (ubyte)((v) & STAT_MASK)))
...
#define GET_PLYR(a) (IS_MORPH(a) ? MORPH_ORIG(a) : ((a)->desc && (a)->desc->original) ? (a)->desc->original : (a))
#define SWITCHED(a) ((a)->desc && (a)->desc->original && (a) == (a)->desc->original)

#define IS_SHOPKEEPER(a) (IS_NPC(a) && ((mob_index[GET_RNUM(a)].qst_func == shop_keeper) || (mob_index[GET_RNUM(a)].func.mob == shop_keeper)))
...
#define CAN_GO(ch, door)                                                                                                                                                                               \
	(ch && EXIT(ch, door) && (EXIT(ch, door)->to_room != NOWHERE) && (!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED) || IS_AFFECTED2(ch, AFF2_PASSDOOR)) &&                                             \
	 !IS_SET(EXIT(ch, door)->exit_info, EX_SECRET | EX_BLOCKED) && !check_castle_walls(ch->in_room, EXIT(ch, door)->to_room))

#define VIRTUAL_CAN_GO(room, door) (VIRTUAL_EXIT(room, door) && (VIRTUAL_EXIT(room, door)->to_room != NOWHERE) && !IS_SET(VIRTUAL_EXIT(room, door)->exit_info, EX_CLOSED | EX_SECRET | EX_BLOCKED))
...
#define IS_OPPOSING_ALIGN(ch, victim) ((IS_GOOD(ch) && IS_EVIL(victim)) || (IS_EVIL(ch) && IS_GOOD(victim)))

#define IS_BACKRANKED(ch) ((ch)->group && IS_SET((ch)->specials.act2, ACT2_BACK_RANK))

#define NPC_REMEMBERS_GROUP(ch) (IS_ELITE(ch) || IS_SET(ch->specials.act2, ACT2_REMEMBERS_GROUP))
...
#define MOB_IN_NO_MAGIC_ROOM(ch) (IS_ROOM(ch->in_room, (ROOM_NO_MAGIC | ROOM_SILENT) || ch->in_room != NOWHERE) || IS_ROOM(ch->in_room, ROOM_SAFE))
...
#define CHAR_IN_HEAL_ROOM(CH) ((CH)->in_room != NOWHERE && (IS_ROOM((CH)->in_room, ROOM_HEAL) || get_spell_from_room(&world[ch->in_room], SPELL_CONSECRATE_LAND)))

#define CHAR_IN_NO_HEAL_ROOM(CH) (((CH)->in_room != NOWHERE) && IS_ROOM((CH)->in_room, ROOM_NO_HEAL))
...
#define LOOP_THRU_PEOPLE(IN_ROOM, PLAYER) for ((IN_ROOM) = world[(PLAYER)->in_room].people; (IN_ROOM) != NULL; (IN_ROOM) = (IN_ROOM)->next_in_room)
...
#define CAN_BREATHE(ch)                                                                                                                                                                                \
	(IS_NPC(ch) && (IS_DRAGON(ch) || IS_AVATAR(ch) || IS_TITAN(ch) ||                                                                                                                                  \
	                IS_ACT(ch, ACT_BREATHES_FIRE | ACT_BREATHES_LIGHTNING | ACT_BREATHES_FROST | ACT_BREATHES_ACID | ACT_BREATHES_GAS | ACT_BREATHES_SHADOW | ACT_BREATHES_BLIND_GAS)))
...
#define IS_GIANT(ch)                                                                                                                                                                                   \
	((GET_RACE(ch) == RACE_GIANT) || (GET_RACE(ch) == RACE_OGRE) || (GET_RACE(ch) == RACE_SGIANT) || (GET_RACE(ch) == RACE_MINOTAUR) || (GET_RACE(ch) == RACE_SNOW_OGRE) ||                            \
	 (GET_RACE(ch) == RACE_FIRBOLG) || (GET_RACE(ch) == RACE_FIREGIANT) || (GET_RACE(ch) == RACE_FROSTGIANT))
...
#define IS_UNDEADRACE(ch)                                                                                                                                                                              \
	((GET_RACE(ch) == RACE_UNDEAD) || (GET_RACE(ch) == RACE_LICH) || (GET_RACE(ch) == RACE_PVAMPIRE) || (GET_RACE(ch) == RACE_SHADE) || (GET_RACE(ch) == RACE_REVENANT) ||                             \
	 (GET_RACE(ch) == RACE_PSBEAST) || (GET_RACE(ch) == RACE_WIGHT) || (GET_RACE(ch) == RACE_PHANTOM) || (IS_DRACOLICH(ch)) || (IS_UNDEAD(ch)))

#define IS_THEURPET_RACE(ch)                                                                                                                                                                           \
	((GET_RACE(ch) == RACE_ARCHON) || (GET_RACE(ch) == RACE_ASURA) || (GET_RACE(ch) == RACE_BRALANI) || (GET_RACE(ch) == RACE_GHAELE) || (GET_RACE(ch) == RACE_ELADRIN) ||                             \
	 (GET_RACE(ch) == RACE_TITAN) || (GET_RACE(ch) == RACE_AVATAR))

#define IS_UNDEAD(ch)                                                                                                                                                                                  \
	((GET_RACE(ch) == RACE_UNDEAD) || (GET_RACE(ch) == RACE_GHOST) || (GET_RACE(ch) == RACE_VAMPIRE) || (GET_RACE(ch) == RACE_LICH) || (GET_RACE(ch) == RACE_PDKNIGHT) ||                              \
	 (GET_RACE(ch) == RACE_ZOMBIE) || (GET_RACE(ch) == RACE_SPECTRE) || (GET_RACE(ch) == RACE_SKELETON) || (GET_RACE(ch) == RACE_WRAITH) || (GET_RACE(ch) == RACE_SHADOW) || (IS_DRACOLICH(ch)) ||     \
	 (IS_AFFECTED(ch, AFF_WRAITHFORM)) || (IS_AFFECTED4(ch, AFF4_VAMPIRE_FORM) && !GET_CLASS(ch, CLASS_THEURGIST)) || (OLD_RACE_PUNDEAD(GET_RACE(ch))))

#define IS_ANGELIC(ch) (IS_AFFECTED4(ch, AFF4_VAMPIRE_FORM) && GET_CLASS(ch, CLASS_THEURGIST))

#define IS_ELEMENTAL(ch)                                                                                                                                                                               \
	((GET_RACE(ch) == RACE_F_ELEMENTAL) || (GET_RACE(ch) == RACE_A_ELEMENTAL) || (GET_RACE(ch) == RACE_W_ELEMENTAL) || (GET_RACE(ch) == RACE_V_ELEMENTAL) || (GET_RACE(ch) == RACE_I_ELEMENTAL) ||     \
	 (GET_RACE(ch) == RACE_E_ELEMENTAL))

#define IS_PC_CORPSE(obj) ((obj->type == ITEM_CORPSE) && IS_SET(obj->value[1], PC_CORPSE))

#define IS_NOCORPSE(ch)                                                                                                                                                                                \
	((GET_RACE(ch) == RACE_UNDEAD) || (GET_RACE(ch) == RACE_GHOST) || (GET_RACE(ch) == RACE_VAMPIRE) || (GET_RACE(ch) == RACE_LICH) || (GET_RACE(ch) == RACE_ZOMBIE) ||                                \
	 (GET_RACE(ch) == RACE_SPECTRE) || (GET_RACE(ch) == RACE_SKELETON) || (GET_RACE(ch) == RACE_WRAITH) || (GET_RACE(ch) == RACE_DEVA) || (GET_RACE(ch) == RACE_SHADOW) ||                             \
	 (GET_RACE(ch) == RACE_W_ELEMENTAL) || (GET_RACE(ch) == RACE_A_ELEMENTAL) || (GET_RACE(ch) == RACE_F_ELEMENTAL) || (GET_RACE(ch) == RACE_E_ELEMENTAL) || (GET_RACE(ch) == RACE_V_ELEMENTAL) ||     \
	 (affected_by_spell(ch, TAG_CONJURED_PET)) || (GET_RACE(ch) == RACE_I_ELEMENTAL) || (IS_THEURPET_RACE(ch)) || (GET_RACE(ch) == RACE_EFREET))

#define IS_ANIMAL(ch)                                                                                                                                                                                  \
	((GET_RACE(ch) == RACE_ANIMAL) || (GET_RACE(ch) == RACE_AQUATIC_ANIMAL) || (GET_RACE(ch) == RACE_FLYING_ANIMAL) || (GET_RACE(ch) == RACE_QUADRUPED) || (GET_RACE(ch) == RACE_PRIMATE) ||           \
	 (GET_RACE(ch) == RACE_HERBIVORE) || (GET_RACE(ch) == RACE_CARNIVORE) || (GET_RACE(ch) == RACE_PARASITE) || (GET_RACE(ch) == RACE_REPTILE) || (GET_RACE(ch) == RACE_DRAGONKIN) ||                  \
	 (GET_RACE(ch) == RACE_SNAKE) || (GET_RACE(ch) == RACE_ARACHNID) || (GET_RACE(ch) == RACE_INSECT))

#define IS_PLANT(ch) ((GET_RACE(ch) == RACE_PLANT) || (GET_RACE(ch) == RACE_SLIME))

#define IS_HUMANOID(ch)                                                                                                                                                                                \
	((GET_RACE(ch) > RACE_NONE) && (GET_RACE(ch) < 31) || (GET_RACE(ch) == RACE_TIEFLING) || (GET_RACE(ch) == RACE_FIRBOLG) || (GET_RACE(ch) == RACE_AGATHINON) || (GET_RACE(ch) == RACE_HUMANOID) ||  \
	 (GET_RACE(ch) == RACE_HALFORC) || (GET_RACE(ch) == RACE_FAERIE) || (GET_RACE(ch) == RACE_PVAMPIRE) || (GET_RACE(ch) == RACE_VAMPIRE) || (GET_RACE(ch) == RACE_RAKSHASA) ||                        \
	 (GET_RACE(ch) == RACE_ANGEL) || (GET_RACE(ch) == RACE_DRIDER) || (GET_RACE(ch) == RACE_SNOW_OGRE) || (GET_RACE(ch) == RACE_LYCANTH) || (GET_RACE(ch) == RACE_ZOMBIE) ||                           \
	 (GET_RACE(ch) == RACE_PRIMATE) || (GET_RACE(ch) == RACE_ELADRIN) || (GET_RACE(ch) == RACE_KOBOLD) || (GET_RACE(ch) == RACE_PILLITHID) || (GET_RACE(ch) == RACE_KUOTOA) ||                         \
	 (GET_RACE(ch) == RACE_WOODELF) || (GET_RACE(ch) == RACE_ARCHON) || (GET_RACE(ch) == RACE_ASURA) || (GET_RACE(ch) == RACE_GHAELE) || (GET_RACE(ch) == RACE_BRALANI) ||                             \
	 (GET_RACE(ch) == RACE_INCUBUS) || (GET_RACE(ch) == RACE_SUCCUBUS))

#define IS_CONTROLLER_RACE(race)                                                                                                                                                                       \
	((race >= RACE_HUMAN && race <= RACE_PLAYER_MAX && race != RACE_ILLITHID) || race == RACE_AGATHINON || race == RACE_HUMANOID || race == RACE_HALFORC || race == RACE_FAERIE ||                     \
	 race == RACE_VAMPIRE || race == RACE_RAKSHASA || race == RACE_ANGEL || race == RACE_SNOW_OGRE || race == RACE_LYCANTH || race == RACE_ZOMBIE || race == RACE_PRIMATE || race == RACE_ELADRIN)
...
#define HAS_FOUR_HANDS(ch) ((GET_RACE(ch) == RACE_THRIKREEN) || (IS_AFFECTED3((ch), AFF3_FOUR_ARMS)))

#define IS_INCORPOREAL(ch) (IS_IMMATERIAL(ch))

#define IS_PURE_CASTER_CLASS(cls)                                                                                                                                                                      \
	((cls) & (CLASS_SORCERER | CLASS_CONJURER | CLASS_ILLUSIONIST | CLASS_SUMMONER | CLASS_NECROMANCER | CLASS_CLERIC | CLASS_SHAMAN | CLASS_BARD | CLASS_DRUID | CLASS_ETHERMANCER |                  \
	          CLASS_THEURGIST | CLASS_BLIGHTER))
#define IS_PARTIAL_CASTER_CLASS(cls) ((cls) & (CLASS_AVENGER))
#define IS_SEMI_CASTER_CLASS(cls)    ((cls) & (CLASS_ANTIPALADIN | CLASS_PALADIN | CLASS_RANGER | CLASS_DRAGOON | CLASS_REAVER | CLASS_AVENGER))
#define IS_CASTER_CLASS(cls)         (IS_PURE_CASTER_CLASS(cls) || IS_SEMI_CASTER_CLASS(cls))
#define IS_BOOK_CLASS(cls)           ((cls) & (CLASS_SORCERER | CLASS_CONJURER | CLASS_NECROMANCER | CLASS_ILLUSIONIST | CLASS_BARD | CLASS_SUMMONER | CLASS_REAVER | CLASS_THEURGIST))
#define IS_PRAYING_CLASS(cls)        ((cls) & (CLASS_CLERIC | CLASS_PALADIN | CLASS_ANTIPALADIN | CLASS_AVENGER))
#define IS_MEMING_CLASS(cls)         (IS_BOOK_CLASS(cls) || ((cls) & CLASS_SHAMAN))

#define USES_COMMUNE(ch)                                                                                                                                                                               \
	((IS_NPC(ch) || GET_PRIME_CLASS(ch, CLASS_DRUID) || GET_PRIME_CLASS(ch, CLASS_RANGER) || GET_PRIME_CLASS(ch, CLASS_DRAGOON) ||                                                                     \
	  (!(IS_CASTER_CLASS(ch->player.m_class) || USES_MANA(ch)) && (GET_SECONDARY_CLASS(ch, CLASS_DRUID) || GET_SECONDARY_CLASS(ch, CLASS_RANGER)))))

#define USES_DEFOREST(ch) (IS_PC(ch) && (GET_PRIME_CLASS(ch, CLASS_BLIGHTER) || (!(IS_CASTER_CLASS(ch->player.m_class) || USES_MANA(ch)) && GET_SECONDARY_CLASS(ch, CLASS_BLIGHTER))))

#define USES_MANA(ch)                                                                                                                                                                                  \
	((GET_RACE(ch) == RACE_PILLITHID) || (GET_RACE(ch) == RACE_ILLITHID) || (GET_PRIME_CLASS(ch, CLASS_PSIONICIST)) || (GET_PRIME_CLASS(ch, CLASS_MINDFLAYER)) ||                                      \
	 (!IS_CASTER_CLASS(ch->player.m_class) && (GET_SECONDARY_CLASS(ch, CLASS_PSIONICIST) || GET_SECONDARY_CLASS(ch, CLASS_MINDFLAYER))))
...
#define USES_SPELL_SLOTS(ch) (USES_COMMUNE(ch) || USES_DEFOREST(ch) || IS_PUNDEAD(ch) || USES_TUPOR(ch) || USES_FOCUS(ch) || IS_ANGEL(ch))
...
#define CAN_SPEAK(ch)                                                                                                                                                                                  \
	(IS_HUMANOID(ch) || IS_DRAGON(ch) || IS_DRAGONKIN(ch) || IS_DEMON(ch) || IS_GIANT(ch) || ((IS_PC_PET(ch) || IS_PC(ch)) && !IS_ANIMAL(ch)) || IS_UNDEAD(ch) || IS_EFREET(ch) || IS_TITAN(ch))
...
#define IS_AGG_CMD(cmd)                                                                                                                                                                                \
	((cmd == CMD_KILL) || (cmd == CMD_HIT) || (cmd == CMD_BACKSTAB) || (cmd == CMD_BASH) || (cmd == CMD_CAST) || (cmd == CMD_RESCUE) || (cmd == CMD_STEAL) || (cmd == CMD_KICK) || (cmd == CMD_USE) || \
	 (cmd == CMD_RECITE) || (cmd == CMD_CIRCLE) || (cmd == CMD_HITALL) || (cmd == CMD_MURDER) || (cmd == CMD_DISARM) || (cmd == CMD_ASSIST) || (cmd == CMD_BODYSLAM) || (cmd == CMD_HEADBUTT) ||       \
	 (cmd == CMD_SPRINGLEAP) || (cmd == CMD_CHANT) || (cmd == CMD_DRAGONPUNCH) || (cmd == CMD_TRAP) || (cmd == CMD_WILL) || (cmd == CMD_STAMPEDE) || (cmd == CMD_CHARGE) || (cmd == CMD_FIRE) ||       \
	 (cmd == CMD_BREATH) || (cmd == CMD_OGRE_ROAR) || (cmd == CMD_BEARHUG) || (cmd == CMD_THROAT_CRUSH))

#define IS_AGG_SKILL(skl)                                                                                                                                                                              \
	((skl == SKILL_BACKSTAB) || (skl == SKILL_BASH) || (skl == SKILL_BUDDHA_PALM) || (skl == SKILL_CIRCLE) || (skl == SKILL_DISARM) || (skl == SKILL_DRAGON_PUNCH) || (skl == SKILL_HEADBUTT) ||       \
	 (skl == SKILL_HITALL) || (skl == SKILL_KICK) || (skl == SKILL_QUIVERING_PALM) || (skl == SKILL_RESCUE) || (skl == SKILL_SPRINGLEAP) || (skl == SKILL_STEAL) || (skl == SKILL_TRAP) ||             \
	 (skl == SONG_CHARMING) || (skl == SONG_FORGETFULNESS) || (skl == SONG_HARMING) || (skl == SONG_SLEEP) || (skl == SKILL_THROAT_CRUSH) || (skl == SKILL_SHIELDPUNCH) || (skl == SKILL_ROUNDKICK) || \
	 (skl == SKILL_DIRT_TOSS) || (skl == SKILL_SWEEPING_THRUST) || (skl == SKILL_INDIRECT_SHOT) || (skl == SKILL_BEARHUG) || (skl == SKILL_FLANK) || (skl == SKILL_KI_STRIKE) ||                       \
	 (skl == SKILL_JIN_TOUCH) || (skl == SKILL_LANCE_CHARGE) || (skl == SKILL_MAUL) || (skl == SKILL_SHIELD_PUNCH) || (skl == SONG_STORMS) || (skl == SKILL_TACKLE) || (skl == SKILL_TRIP) ||          \
	 (skl == SKILL_WHIRLWIND) || (skl == SKILL_GAZE) || (skl == SKILL_TRAMPLE) || (skl == SONG_DISCORD) || (skl == SONG_HARMONY) || (skl == SKILL_RESTRAIN))
...
#define OLD_RACE_NEUTRAL(race) ((race == RACE_THRIKREEN) || (race == RACE_MINOTAUR) || (race == RACE_TIEFLING))

#define OLD_RACE_GOOD(race, align)                                                                                                                                                                     \
	((race == RACE_HUMAN) || (race == RACE_GREY) || (race == RACE_MOUNTAIN) || (race == RACE_BARBARIAN) || (race == RACE_GNOME) || (race == RACE_HALFLING) || (race == RACE_HALFELF) ||                \
	 (race == RACE_CENTAUR) || (race == RACE_GITHZERAI) || (race == RACE_AGATHINON) || (race == RACE_WOODELF) || (race == RACE_FIRBOLG) || (race == RACE_ELADRIN) ||                                   \
	 (OLD_RACE_NEUTRAL(race) && (align >= 0)))

#define OLD_RACE_EVIL(race, align)                                                                                                                                                                     \
	((race == RACE_DROW) || (race == RACE_DUERGAR) || (race == RACE_GITHYANKI) || (race == RACE_OGRE) || (race == RACE_GOBLIN) || (race == RACE_ORC) || (race == RACE_OROG) || (race == RACE_TROLL) || \
	 (race == RACE_KUOTOA) || (race == RACE_KOBOLD) || (race == RACE_DRIDER) || (OLD_RACE_NEUTRAL(race) && (align < 0)))

#define OLD_RACE_PUNDEAD(race)                                                                                                                                                                         \
	((race == RACE_LICH) || (race == RACE_PVAMPIRE) || (race == RACE_PDKNIGHT) || (race == RACE_SHADE) || (race == RACE_REVENANT) || (race == RACE_PSBEAST) || (race == RACE_WIGHT) ||                 \
...
#define ALONE(ch) (!(ch) || ((ch)->in_room == NOWHERE) || (((ch)->next_in_room == NULL) && (world[(ch)->in_room].people == (ch))))

#define IS_ENCRUSTED(obj) (IS_SET(obj->extra_flags, ITEM_ENCRUSTED))

#define WIZ_INVIS(viewer, target) (IS_TRUSTED(target) && (GET_LEVEL(viewer) <= target->only.pc->wiz_invis) && !is_linked_to(viewer, target, LNK_CONSENT))
...
#define HAS_FOOTING(ch)                                                                                                                                                                                \
	((world[(ch)->in_room].sector_type != SECT_WATER_NOSWIM) && (world[(ch)->in_room].sector_type != SECT_WATER_SWIM) && (world[(ch)->in_room].sector_type != SECT_NO_GROUND) &&                       \
	 (world[(ch)->in_room].sector_type != SECT_UNDERWATER) && (world[(ch)->in_room].sector_type != SECT_FIREPLANE) && (world[(ch)->in_room].sector_type != SECT_OCEAN) &&                              \
	 (world[(ch)->in_room].sector_type != SECT_UNDRWLD_NOSWIM) && (world[(ch)->in_room].sector_type != SECT_UNDRWLD_NOGROUND) && (world[(ch)->in_room].sector_type != SECT_AIR_PLANE) &&               \
	 (world[(ch)->in_room].sector_type != SECT_WATER_PLANE) && (world[(ch)->in_room].sector_type != SECT_ETHEREAL) && (world[(ch)->in_room].sector_type != SECT_ASTRAL) &&                             \
...
#define IS_GREATER_RACE(ch)                                                                                                                                                                            \
	(IS_NPC(ch) && ((GET_RACE(ch) == RACE_DRAGON) || (GET_RACE(ch) == RACE_DEMON) || (GET_RACE(ch) == RACE_DEVIL) || (GET_RACE(ch) == RACE_ANGEL) || (GET_RACE(ch) == RACE_BEHOLDER) ||                \
	                (GET_RACE(ch) == RACE_LICH) || (GET_RACE(ch) == RACE_AVATAR) || (GET_RACE(ch) == RACE_CONSTRUCT)))
...
#define IS_DAYBLIND(ch) (!has_innate(ch, INNATE_EYELESS) && !IS_TRUSTED(ch) && !IS_TWILIGHT_ROOM(ch->in_room) && !IS_AFFECTED4(ch, AFF4_GLOBE_OF_DARKNESS) && has_innate(ch, INNATE_DAYBLIND))
...
#define CAN_SING(ch) (!IS_IMMOBILE(ch) && !is_silent(ch, FALSE) && !IS_STUNNED(ch) && GET_STAT(ch) >= STAT_RESTING)

#define IS_IMMOBILE(ch)                                                                                                                                                                                \
	(IS_AFFECTED2(ch, AFF2_MAJOR_PARALYSIS) || IS_AFFECTED2(ch, AFF2_MINOR_PARALYSIS) || IS_AFFECTED(ch, AFF_KNOCKED_OUT) || IS_AFFECTED(ch, AFF_BOUND) || affected_by_spell(ch, SONG_SLEEP) ||        \
...
#define MAX_CON_BONUS_CLASSES                                                                                                                                                                          \
	(CLASS_DRUID | CLASS_BLIGHTER | CLASS_CLERIC | CLASS_SORCERER | CLASS_NECROMANCER | CLASS_SHAMAN | CLASS_PSIONICIST | CLASS_MINDFLAYER | CLASS_ILLUSIONIST | CLASS_CONJURER | CLASS_SUMMONER |     \
	 CLASS_BARD | CLASS_ETHERMANCER | CLASS_THEURGIST)

#define IS_COLD_VULN(ch) (GET_RACE(ch) == RACE_THRIKREEN || GET_RACE(ch) == RACE_F_ELEMENTAL || GET_RACE(ch) == RACE_EFREET || FIRESHIELDED(ch) || IS_AFFECTED2(ch, AFF2_FIRE_AURA))

#define ENJOYS_FIRE_DAM(ch) ((GET_RACE(ch) == RACE_F_ELEMENTAL || GET_RACE(ch) == RACE_EFREET) || IS_AFFECTED2(ch, AFF2_FIRE_AURA))

#define IS_AFFLICTED_PSI(ch)                                                                                                                                                                           \
	((affected_by_spell(ch, SPELL_POISON) || IS_AFFECTED2(ch, AFF2_POISONED) || affected_by_spell(ch, SPELL_CURSE) || (affected_by_spell(ch, SPELL_WITHER) && GET_LEVEL(ch) > 50) ||                   \
	  (affected_by_spell(ch, SPELL_DISEASE) && GET_LEVEL(ch) > 55) || IS_AFFECTED(ch, AFF_BLIND)) &&                                                                                                   \
...
#define DO_SHAM_RESTORATION(victim)                                                                                                                                                                    \
	(affected_by_spell(victim, SPELL_CURSE) || affected_by_spell(victim, SPELL_MALISON) || affected_by_spell(victim, SPELL_WITHER) || affected_by_spell(victim, SPELL_BLOODTOSTONE) ||                 \
	 affected_by_spell(victim, SPELL_SHREWTAMENESS) || affected_by_spell(victim, SPELL_MOUSESTRENGTH) || affected_by_spell(victim, SPELL_MOLEVISION) || affected_by_spell(victim, SPELL_SNAILSPEED) || \
	 affected_by_spell(victim, SPELL_FEEBLEMIND) || affected_by_spell(victim, SPELL_SLOW) || IS_AFFECTED(victim, AFF_BLIND) || IS_AFFECTED4(victim, AFF4_CARRY_PLAGUE) ||                              \
	 affected_by_spell(victim, SPELL_DISEASE) || affected_by_spell(victim, SPELL_PLAGUE) || affected_by_spell(victim, TAG_ARMLOCK) || affected_by_spell(victim, TAG_LEGLOCK) ||                        \
	 affected_by_spell(victim, SPELL_BMANTLE) || affected_by_spell(victim, SPELL_ENERGY_DRAIN) || affected_by_spell(victim, SPELL_FLAMESTRIKE))
...
#define SHOCK_REAVER_WEAPONS(wpn)                                                                                                                                                                      \
	((IS_SWORD(wpn) && !IS_SET(wpn->extra_flags, ITEM_TWOHANDS)) || (IS_AXE(wpn) && !IS_SET(wpn->extra_flags, ITEM_TWOHANDS)) || (IS_DAGGER(wpn) && !IS_SET(wpn->extra_flags, ITEM_TWOHANDS)))
...
#define SHAMAN_MOVE_SPELL(ch) (affected_by_spell(ch, SPELL_WOLFSPEED) || affected_by_spell(ch, SPELL_SNAILSPEED) || affected_by_spell(ch, SPELL_PANTHERSPEED))

#define SHAMAN_STR_SPELL(ch) (affected_by_spell(ch, SPELL_MOUSESTRENGTH) || affected_by_spell(ch, SPELL_BEARSTRENGTH) || affected_by_spell(ch, SPELL_ELEPHANTSTRENGTH))

#define IS_GREATER_ELEMENTAL(mob)                                                                                                                                                                      \
	(IS_NPC(mob) && (GET_VNUM(mob) >= 1110 && GET_VNUM(mob) <= 1112 || GET_VNUM(mob) >= 1120 && GET_VNUM(mob) <= 1122 || GET_VNUM(mob) >= 1130 && GET_VNUM(mob) <= 1132 ||                             \
	                 GET_VNUM(mob) >= 1140 && GET_VNUM(mob) <= 1142 || GET_VNUM(mob) >= 41 && GET_VNUM(mob) <= 44))
...
#define IS_BRAINLESS(victim)                                                                                                                                                                           \
	((GET_RACE(victim) == RACE_PLANT) || (GET_RACE(victim) == RACE_GOLEM) || (GET_RACE(victim) == RACE_CONSTRUCT) || (GET_RACE(victim) == RACE_SLIME) || (GET_RACE(victim) == RACE_PWORM) ||           \
	 (GET_RACE(victim) == RACE_SKELETON) || (GET_RACE(victim) == RACE_ZOMBIE) || (IS_NPC(victim) && GET_VNUM(victim) == 250))

#define IS_SOULLESS(victim) ((GET_RACE(victim) == RACE_PLANT) || (GET_RACE(victim) == RACE_GOLEM) || IS_UNDEADRACE(victim) || (GET_RACE(victim) == RACE_CONSTRUCT) || (GET_RACE(victim) == RACE_SLIME))

#define IS_CONSTRUCT(ch) ((GET_RACE(ch) == RACE_GOLEM) || (GET_RACE(ch) == RACE_CONSTRUCT))

#define IS_NECRO_GOLEM(ch)                                                                                                                                                                             \
	IS_NPC(ch) && (GET_RACE(ch) == RACE_GOLEM) && (GET_VNUM(ch) == golem_data[0].vnum || GET_VNUM(ch) == golem_data[1].vnum || GET_VNUM(ch) == golem_data[2].vnum || GET_VNUM(ch) == golem_data[3].vnum)

#define IS_ANGEL(ch)                                                                                                                                                                                   \
	((GET_RACE(ch) == RACE_ANGEL) || (GET_RACE(ch) == RACE_ELADRIN) || (GET_RACE(ch) == RACE_AGATHINON) || (GET_RACE(ch) == RACE_ARCHON) || (GET_RACE(ch) == RACE_ASURA) ||                            \
	 (GET_RACE(ch) == RACE_TITAN) || (GET_RACE(ch) == RACE_AVATAR) || (GET_RACE(ch) == RACE_GHAELE) || (GET_RACE(ch) == RACE_BRALANI) || (GET_RACE(ch) == RACE_DEVA) || IS_ANGELIC(ch))
...
#define INFRA_INVIS_RACE(race)                                                                                                                                                                         \
	(race == RACE_UNDEAD || race == RACE_GHOST || race == RACE_VAMPIRE || race == RACE_LICH || race == RACE_PDKNIGHT || race == RACE_ZOMBIE || race == RACE_SPECTRE || race == RACE_SKELETON ||        \
	 race == RACE_WRAITH || race == RACE_SHADOW || race == RACE_DRACOLICH || race == RACE_PVAMPIRE || race == RACE_SHADE || race == RACE_REVENANT || race == RACE_PSBEAST || race == RACE_WIGHT ||     \
	 race == RACE_GARGOYLE || race == RACE_PHANTOM || race == RACE_AQUATIC_ANIMAL || race == RACE_PARASITE || race == RACE_GOLEM || race == RACE_SNAKE || race == RACE_ARACHNID ||                     \
	 race == RACE_PLANT || race == RACE_SLIME || race == RACE_A_ELEMENTAL || race == RACE_INSECT || race == RACE_W_ELEMENTAL || race == RACE_V_ELEMENTAL || race == RACE_I_ELEMENTAL ||                \
	 race == RACE_E_ELEMENTAL || (IS_AFFECTED4(obj, AFF4_VAMPIRE_FORM) && !GET_CLASS(obj, CLASS_THEURGIST)) || IS_RACEWAR_UNDEAD(obj))

// This is done in reverse 'cause it's faster that way.
#define HAS_LUNGS(race)                                                                                                                                                                                \
	(race != RACE_SHADE && race != RACE_REVENANT && race != RACE_LICH && race != RACE_PVAMPIRE && race != RACE_PDKNIGHT && race != RACE_PSBEAST && race != RACE_WIGHT && race != RACE_PHANTOM &&       \
	 race != RACE_GARGOYLE && race != RACE_F_ELEMENTAL && race != RACE_A_ELEMENTAL && race != RACE_UNDEAD && race != RACE_VAMPIRE && race != RACE_W_ELEMENTAL && race != RACE_E_ELEMENTAL &&           \
	 race != RACE_GHOST && race != RACE_GOLEM && race != RACE_PLANT && race != RACE_DRACOLICH && race != RACE_SLIME && race != RACE_CONSTRUCT && race != RACE_ZOMBIE && race != RACE_SPECTRE &&        \
	 race != RACE_SKELETON && race != RACE_WRAITH && race != RACE_V_ELEMENTAL && race != RACE_I_ELEMENTAL && race != RACE_SHADOW && race != RACE_AVATAR)
...
#define COMBAT_PULSE(ch)                                                                                                                                                                               \
	(ch->specials.base_combat_round + ch->points.combat_pulse * ((.04 / 3) * ch->specials.base_combat_round * ch->specials.base_combat_round - (1.10 / 3) * ch->specials.base_combat_round + 2.73))
...
#define ITEM_LOAD_CHECK(item, ival, zone_percent) item_load_check(item, ival, zone_percent)
