...
void show_guild_frags(P_char ch)
{
	...
	while (i < guild_count)
	{
		snprintf(Gbuf1,
		         ...
		         "    &+L---------------------------------------\n",
		         gfrag_list[i].guild_name,
		         gfrag_list[i].tot_frags / 100.00,
		         gfrag_list[i].top_fragger,
		         gfrag_list[i].top_frags / 100.00,
...
Guild::Guild(char *_name, unsigned int _racewar, unsigned int _id_number, unsigned long _prestige, unsigned long _construction, unsigned long _money, unsigned int _bits)
{
	strcpy(name, _name);
	racewar      = _racewar;
...
bool Guild::load_guild(int guild_num)
{
	...
	*strchrnul(new_guild->name, '\n') = '\0';
	...
	fscanf(file, "%u %lu %lu %s\n", &(new_guild->racewar), &(new_guild->frags.frags), &(new_guild->frags.top_frags), new_guild->frags.topfragger);
	...
	fscanf(file, "%u %u %u %u\n", &(new_guild->platinum), &(new_guild->gold), &(new_guild->silver), &(new_guild->copper));
...
bool Guild::add_member(P_char ch, int rank)
{
	...
	if (GET_ASSOC(ch) != NULL)
	{
		...
		debug("Guild::add_member: '%s' %d is trying to enter guild '%s' %d, but already in guild '%s' %d.", J_NAME(ch), GET_ID(ch), name, id_number, GET_ASSOC(ch)->name, GET_ASSOC(ch)->id_number);
		return FALSE;
	}
...
void Guild::update_member(P_char ch)
{
	...
	if ((gh = Guildhall::find_by_vnum(GET_BIRTHPLACE(ch))) != NULL)
	{
		...
		{
			logit(LOG_DEBUG, "%s was homed in GH but doesn't belong to guild, resetting to original home", GET_NAME(ch));
...
void Guild::kick(P_char ch)
{
	...
	{
		debug("Trying to kick '%s' %d from guild '%d' when they are in guild '%d'", J_NAME(ch), GET_ID(ch), get_id(), GET_ASSOC(ch)->get_id());
		return;
	}
...
bool Guild::is_enemy(P_char enemy)
{
	...
	// Control if enemy
	while (fscanf(f, "%s %u %u %u %u %u\n", buf, &dummy1, &dummy2, &dummy3, &dummy4, &dummy5) != EOF)
	{
...
void Guild::display(P_char member)
{
	...
	const char *standard_names[ASC_NUM_RANKS] = {"enemy", "parole", "normal", "senior", "officer", "deputy", "leader", "king"};
	P_member    pMembers;
	...
	snprintf(
		buf + strlen(buf), MAX_STRING_LENGTH - strlen(buf), "Total Frags: &+W%.2f&N, Top Fragger: '&+W%s&N' with &+W%.2f&N frags.\n", frags.frags / 100., frags.topfragger, frags.top_frags / 100.);
			...
			while (pMembers)
			{
				if (NR_RANK(pMembers->bits) == i)
				{
					snprintf(buf + strlen(buf),
					         ...
					         : (pMembers->online_status == GSTAT_LINKDEAD) ? "&+y+&n"
					                                                       : " ",
					         pMembers->name,
					         current_title,
					         (pMembers->debt && ((rank > A_SENIOR) || !strcmp(pMembers->name, GET_NAME(member))))
					             ? (((rank > GET_RK_BITS(pMembers->bits)) || !strcmp(pMembers->name, GET_NAME(member))) ? coin_stringv(pMembers->debt, 0) : "&+R(-)&n")
					             : "");
				}
				pMembers = pMembers->next;
			}
...
void supervise_list(P_char god)
{
	...
	strcpy(buf,
	       " Number : Racewar : Name of Player Associations\n"
	       " ----------------------------------------------\n");
...
void do_supervise(P_char god, char *argument, int cmd)
{
	...
	if (is_abbrev(first, "found"))
	{
		...
		wizlog(GET_LEVEL(god), "%s made %s founding leader of %s, bits string is '%s'.", GET_NAME(god), GET_NAME(member), rest, third);
		logit(LOG_WIZ, "%s made %s founding leader of %s, bits string is '%s'.", GET_NAME(god), GET_NAME(member), rest, third);
		sql_log(god, WIZLOG, "%s made %s founding leader of %s, bits string is '%s'.", GET_NAME(god), GET_NAME(member), rest, third);
		return;
	}
	...
	if (is_abbrev(first, "type"))
	{
		...
		guild->set_bits(bits);
		send_to_char_f(god, "The new bits string for '%s' %d is: %b.", guild->get_name().c_str(), guild->get_id(), bits);
		return;
	}
	...
	if (is_abbrev(first, "setbit"))
	{
		...
		else if (is_abbrev(third, "racewar"))
		{
			...
			guild->save();
			send_to_char_f(god, "You set %s's racewar to %d - &+%c%s&n.\n", guild->get_name().c_str(), bits, racewar_color[bits].color, racewar_color[bits].name);
			return;
		}
		else
		{
...
void Guild::rank(P_char member, P_char victim, char *rank_change)
{
	...
	if ((GET_RK_BITS(memb_bits) < A_LEADER) && (GET_RK_BITS(memb_bits) == GET_RK_BITS(vict_bits)) && (rank_delta < 0))
	{
	...
	send_to_char_f(member, "You %s %s to %s %s.\n", ((rank_delta < 0) ? "demoted" : "promoted"), vict_name, titles[NR_RANK(GET_A_BITS(victim))], name);
...
void Guild::secede(P_char member)
{
	...
	{
		debug("Trying to secede '%s' %d from guild '%d' when they are in guild '%d'", J_NAME(member), GET_ID(member), get_id(), GET_ASSOC(member)->get_id());
		return;
	}
