...
void do_track_not_in_use(P_char ch, char *arg, int cmd)
{
	...
	for (obj = world[ch->in_room].contents; obj; obj = obj->next_content)
	{
		if (obj->R_num == real_object(4))
		{
			...
			else if ((obj->value[1] == BLOOD_REG) && (epic_skill >= number(50, 70)))
			{
				snprintf(descbuf, MAX_STRING_LENGTH, "%s\n", obj->description);
				send_to_char(descbuf, ch);
				found_track = TRUE;
			}
			else if ((obj->value[1] == BLOOD_DRY) && (epic_skill >= number(90, 100)))
			{
				snprintf(descbuf, MAX_STRING_LENGTH, "%s\n", obj->description);
				send_to_char(descbuf, ch);
				found_track = TRUE;
...
int MaxTrackDist(P_char ch)
{
	...
	if (GET_CLASS(ch, CLASS_ROGUE) || GET_CLASS(ch, CLASS_THIEF) || GET_CLASS(ch, CLASS_RANGER))
		dist = (int)(dist * 1.5);
